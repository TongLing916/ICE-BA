...
DEFINE_int32(ft_len, 125,
             "The feature track length threshold when dropout kicks in");
DEFINE_double(ft_droprate, 0.05,
              "The drop out rate when a feature track exceeds ft_len");
...
DEFINE_string(gba_camera_save_path, "",
              "Save the camera states to when finished");
...
void load_asl_calib(const std::string& asl_path,
  ...
  calib_param.Camera.cv_camK_lr[0] << v_float[0], 0, v_float[2], 0, v_float[1],
      v_float[3], 0, 0, 1;
  calib_param.Camera.cameraK_lr[0] << v_float[0], 0, v_float[2], 0, v_float[1],
      v_float[3], 0, 0, 1;
  v_float = cam1_calib["intrinsics"].as<std::vector<float>>();
  calib_param.Camera.cv_camK_lr[1] << v_float[0], 0, v_float[2], 0, v_float[1],
      v_float[3], 0, 0, 1;
  calib_param.Camera.cameraK_lr[1] << v_float[0], 0, v_float[2], 0, v_float[1],
      v_float[3], 0, 0, 1;
  ...
  calib_param.Camera.cv_dist_coeff_lr[0] =
      (cv::Mat_<float>(8, 1) << static_cast<float>(v_double[0]),
       static_cast<float>(v_double[1]), static_cast<float>(v_double[2]),
       static_cast<float>(v_double[3]), 0.0, 0.0, 0.0, 0.0);
...
bool convert_to_asl_timestamp(const string& file_in, const string& file_out,
                              uint64_t offset_ns) {
  ...
  float x, y, z;
  float qx, qy, qz, qw;
  while (fscanf(fp_in, "%f %f %f %f %f %f %f %f", &t, &x, &y, &z, &qx, &qy, &qz,
                &qw) == 8) {
...
bool create_iba_frame(const vector<cv::KeyPoint>& kps_l,
  ...
  CF.iFrm = iba_iFrm;
  InitPOD(CF.C);  // needed to ensure the dumped frame deterministic even for
                  // unused field
  ...
  std::transform(imu_samples.begin(), imu_samples.end(),
                 std::back_inserter(CF.us), XP::to_iba_imu);
  CF.t = rig_time;
  CF.d = kUnknownDepth;
  bool need_new_kf =
      std::distance(kp_it_l, kps_l.end()) >= 20 || CF.zs.size() < 20;
  if (std::distance(kp_it_l, kps_l.end()) == 0) need_new_kf = false;
  if (!need_new_kf)
    KF.iFrm = -1;
  else
...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < limg_name.size(); i++) {
    ...
    slave_img_file_paths.push_back(FLAGS_imgs_folder + "/mav0/cam1/data/" +
                                   rimg_name[i]);
    iba_dat_file_paths.push_back(FLAGS_imgs_folder + "/dat/" + limg_name[i] +
                                 ".dat");
  }
  if (!FLAGS_stereo) {
  ...
  for (int lr = 0; lr < 2; ++lr) {
    ...
    if (XP::generate_cam_mask(duo_calib_param.Camera.cv_camK_lr[lr],
                              ...
                              duo_calib_param.Camera.img_size, &masks[lr],
                              &fov)) {
  ...
  while (FLAGS_start_idx < FLAGS_end_idx &&
         get_timestamp_from_img_name(img_file_paths[FLAGS_start_idx],
                                     offset_ts_ns) <=
             imu_samples.front().time_stamp)
    FLAGS_start_idx++;

  XP::FeatureTrackDetector feat_track_detector(
      FLAGS_ft_len, FLAGS_ft_droprate, !FLAGS_not_use_fast,
      FLAGS_uniform_radius, duo_calib_param.Camera.img_size);
  XP::ImgFeaturePropagator slave_img_feat_propagator(
      ...
      masks[1], FLAGS_pyra_level,
      FLAGS_min_feature_distance_over_baseline_ratio,
      FLAGS_max_feature_distance_over_baseline_ratio);
  const Eigen::Matrix4f T_Cl_Cr = duo_calib_param.Camera.D_T_C_lr[0].inverse() *
  ...
  solver.Create(to_iba_calibration(duo_calib_param), 257, IBA_VERBOSE_NONE,
                IBA_DEBUG_NONE, 257, FLAGS_iba_param_path,
                "" /* iba directory */);
  ...
  for (int it_img = FLAGS_start_idx; it_img < FLAGS_end_idx; ++it_img) {
    ...
    // Get the imu measurements within prev_time_stamp and time_stamp to compute
    // old_R_new
    imu_meas.reserve(10);
    ...
    if (it_img != FLAGS_start_idx) {
      ...
      if (imu_meas.size() > 1) {
        // Here we simply the transformation chain to rotation only and assume
        // zero translation
        cv::Matx33f old_R_new;
        XP::XpQuaternion
            I_new_q_I_old;  // The rotation between the new {I} and old {I}
        for (size_t i = 1; i < imu_meas.size(); ++i) {
          ...
          XP::IntegrateQuaternion(
              ...
              imu_meas[i].time_stamp - imu_meas[i - 1].time_stamp, &q_end);
          I_new_q_I_old = q_end;
        }
        ...
        feat_track_detector.optical_flow_and_detect(
            masks[0], pre_image_features, pre_image_key_points,
            request_feat_num, FLAGS_pyra_level, FLAGS_fast_thresh, &key_pnts,
            &orb_feat, cv::Vec2f(0, 0),  // shift init pixels
      ...
      feat_track_detector.detect(
          img_in_smooth, masks[0],
          FLAGS_max_num_per_grid * FLAGS_grid_row_num * FLAGS_grid_col_num,
          FLAGS_pyra_level, FLAGS_fast_thresh, &key_pnts, &orb_feat);
      feat_track_detector.build_img_pyramids(
