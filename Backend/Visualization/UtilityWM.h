...
template <>
inline CVD::Rgb<ubyte> Invalid<CVD::Rgb<ubyte> >() {
...
inline bool ImageValid(const CVD::Image<TYPE> &I) {
  return ImageValid(I.size());
}
template <typename TYPE>
inline bool ImageInvalid(const CVD::Image<TYPE> &I) {
  return I.size().x == 0 || I.size().y == 0;
}
template <typename TYPE>
inline void ImageInvalidate(CVD::Image<TYPE> &I) {
...
inline void ImageCopy(const CVD::Image<TYPE_SRC> &Is,
...
//  0x00ff00ff);
//  const int w = Is.size().x >> 1, h = Is.size().y >> 1, wF =
//  SIMD_BYTE_FLOOR(w);
...
//      zero));
//      gy = _mm_sub_epi16(_mm_unpacklo_epi8(*i2, zero), _mm_unpacklo_epi8(*i0,
//      zero));
//      *g1++ = _mm_unpacklo_epi16(gx, gy);
//      *g1++ = _mm_unpackhi_epi16(gx, gy);
//      gx = _mm_sub_epi16(_mm_unpackhi_epi8(i12, zero), _mm_unpackhi_epi8(i10,
//      zero));
//      gy = _mm_sub_epi16(_mm_unpackhi_epi8(*i2, zero), _mm_unpackhi_epi8(*i0,
//      zero));
//      *g1++ = _mm_unpacklo_epi16(gx, gy);
//      *g1++ = _mm_unpackhi_epi16(gx, gy);
//#else
...
//      _mm_unpacklo_epi8(i10.v, zero));
//      gy = _mm_sub_epi16(_mm_unpacklo_epi8(*i2, zero), _mm_unpacklo_epi8(*i0,
//      zero));
//      *g1++ = _mm_unpacklo_epi16(gx, gy);
//      *g1++ = _mm_unpackhi_epi16(gx, gy);
//      gx = _mm_sub_epi16(_mm_unpackhi_epi8(i12.v, zero),
//      _mm_unpackhi_epi8(i10.v, zero));
//      gy = _mm_sub_epi16(_mm_unpackhi_epi8(*i2, zero), _mm_unpackhi_epi8(*i0,
//      zero));
//      *g1++ = _mm_unpacklo_epi16(gx, gy);
//      *g1++ = _mm_unpackhi_epi16(gx, gy);
...
//        _mm_cmpneq_ps(i10, iInv)), gx);
//        gy = _mm_and_ps(_mm_and_ps(_mm_cmpneq_ps(*i2, iInv),
//        _mm_cmpneq_ps(*i0, iInv)), gy);
//      }
...
inline void ImageGradientX(const CVD::Image<TYPE_GRADIENT_XY> &G,
                           CVD::Image<TYPE_GRADIENT> &Gx) {
  Gx.resize(G.size());
...
template <typename TYPE_GRADIENT_XY, typename TYPE_GRADIENT>
inline void ImageGradientY(const CVD::Image<TYPE_GRADIENT_XY> &G,
                           CVD::Image<TYPE_GRADIENT> &Gy) {
  Gy.resize(G.size());
  const int w = G.size().x, h = G.size().y;
  for (int y = 0; y < h; ++y) {
    for (int x = 0; x < w; ++x) {
...
inline void ImageInterpolate<CVD::Rgb<ubyte>, CVD::Rgb<ubyte> >(
  ...
  ImageInterpolate<ubyte, ubyte>(i11.green, i12.green, i21.green, i22.green, w,
                                 ip.green);
  ImageInterpolate<ubyte, ubyte>(i11.blue, i12.blue, i21.blue, i22.blue, w,
                                 ip.blue);
}
template <typename TYPE_SRC, typename TYPE_DST>
...
inline bool ImageInterpolateGradient(const CVD::Image<TYPE_INTENSITY> &I,
                                     const CVD::Image<TYPE_GRADIENT_VECTOR> &G,
  ...
  const TYPE_INTENSITY i11 = I[iy1][ix1], i12 = I[iy2][ix1], i21 = I[iy1][ix2],
                       i22 = I[iy2][ix2];
  if (chkInv &&
  ...
  ImageInterpolate<TYPE_GRADIENT, float>(g11[0], g12[0], g11[2], g12[2], w,
                                         gp[0]);
  ImageInterpolate<TYPE_GRADIENT, float>(g11[1], g12[1], g11[3], g12[3], w,
                                         gp[1]);
  return true;
}
template <typename TYPE>
...
template <typename TYPE>
inline void ImageLoadB(CVD::Image<TYPE> &I, FILE *fp) {
...
template <class TYPE>
inline float ImageMemoryMB(const CVD::Image<TYPE> &I) {
  return MemoryMB<TYPE>(I.totalsize());
}
template <typename TYPE>
inline void ImageDrawLine(CVD::Image<TYPE> &I, const float *x1, const float *x2,
                          const TYPE clr) {
  CVD::drawLine(I, double(x1[0]), double(x1[1]), double(x2[0]), double(x2[1]),
                clr);
}
template <typename TYPE>
...
template <typename TYPE>
inline void ImageDrawBox(CVD::Image<TYPE> &I, const CVD::ImageRef &x,
                         const int size, const TYPE clr) {
...
template <typename TYPE>
inline void ImageDrawBox(CVD::Image<TYPE> &I, const float *x, const int size,
                         const TYPE clr) {
  const int w = I.size().x, h = I.size().y;
