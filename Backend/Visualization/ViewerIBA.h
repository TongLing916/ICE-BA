...
class ViewerIBA : public Viewer {
 public:
  virtual void Create(IBA::Solver *solver, const std::string screenFile = "",
                      const int screenCombine = false,
                      const std::string saveFile = "", const int iFrmSave = -1,
                      const bool wnd = true);
  virtual void Reset();
    ...
    class Measurement {
     public:
      inline bool operator==(const Measurement &iz) const {
        return m_iFrm == iz.m_iFrm && m_iKF == iz.m_iKF && m_iLF == iz.m_iLF &&
               m_iz == iz.m_iz;
      }
      inline bool operator!=(const Measurement &iz) const {
        return m_iFrm != iz.m_iFrm || m_iKF != iz.m_iKF || m_iLF != iz.m_iLF ||
 ...
 protected:
  ...
  virtual Depth::InverseGaussian GetFrameDepthKF(const int iKF,
                                                 const int type = -1);
  virtual Depth::InverseGaussian GetFrameDepthLF(const int iLF,
                                                 const int type = -1);
  virtual Depth::InverseGaussian GetFeatureDepth(const int iKF, const int ix,
                                                 const int type = -1);
  ...
  virtual bool DragActiveFrame(const CVD::ImageRef &from,
                               const CVD::ImageRef &to);
  virtual void ActivateFeature(const FeatureIndex::Source &ix,
                               const int iz = -1);
  virtual bool SearchActiveFeature(const CVD::ImageRef &x);
  virtual void SearchActiveFeature(const Point2D &x, const int iKF = -1,
                                   int ix = -1, const int iz = -1);
  ...
  virtual int CountFeatureMatchesMeasurement(const FRM::Frame &F1,
                                             const FRM::Frame &F2);
  ...
  virtual void Get2DProjectionRange(const Point2D &x, const LA::Vector2f &Jx,
  ...
  virtual void Draw3DDepthPlane(const Rigid3D &C, const float d,
                                Point3D *Xs = NULL);
  ...
  virtual void DrawProfileTypeStep(const int *types, const int N,
                                   const bool next);
  ...
  virtual bool OnMouseDraging(const CVD::ImageRef &from,
                              const CVD::ImageRef &to, const int button);
  virtual bool OnMouseDoubleClicked(const CVD::ImageRef &where,
                                    const int button);
  virtual void OnResize(const CVD::ImageRef &size);
  virtual void SaveB(FILE *fp);
  virtual void LoadB(FILE *fp);
  virtual void SaveScreen(const std::string fileName,
                          const bool sizeImg = true);
