...
class EventHandler : public CVD::GLWindow::EventHandler {
 public:
  ...
  typedef void (HANDLER::*CallBackMouseDown)(const CVD::ImageRef &where,
                                             const int button);
  typedef void (HANDLER::*CallBackMouseUp)(const CVD::ImageRef &where,
                                           const int button);
  typedef bool (HANDLER::*CallBackMouseDraging)(const CVD::ImageRef &from,
                                                const CVD::ImageRef &to,
                                                const int button);
  typedef bool (HANDLER::*CallBackMouseDoubleClick)(const CVD::ImageRef &where,
                                                    const int button);
 ...
 public:
  inline EventHandler()
      : m_quit(false),
        ...
        m_mouseDownCB(NULL),
        m_mouseUpCB(NULL),
        m_mouseDragingCB(NULL),
        m_mouseDoubleClickCB(NULL),
        m_resizeCB(NULL) {}
  ...
  virtual void on_key_down(CVD::GLWindow &win, int key) {
    ...
    if (m_handler && m_keyDownCB) (m_handler->*m_keyDownCB)(key);
  }
  /// Called for key release events
  virtual void on_key_up(CVD::GLWindow &win, int key) {
    m_keyDown = false;
    m_key = -1;
    if (m_handler && m_keyUpCB) (m_handler->*m_keyUpCB)(key);
  }
  /// Called for mouse movement events
  virtual void on_mouse_move(CVD::GLWindow &win, CVD::ImageRef where,
                             int state) {
  ...
  virtual void on_mouse_down(CVD::GLWindow &win, CVD::ImageRef where, int state,
                             int button) {
    m_mouseDown = true;
    m_button = button;
  ...
  virtual void on_mouse_up(CVD::GLWindow &win, CVD::ImageRef where, int state,
                           int button) {
    if ((button == VW_MOUSE_BUTTON_LEFT || button == VW_MOUSE_BUTTON_RIGHT) &&
      ...
      if (m_handler && m_mouseUpCB) (m_handler->*m_mouseUpCB)(where, button);
    }
    m_mouseDown = false;
  ...
  virtual void on_resize(CVD::GLWindow &win, CVD::ImageRef size) {
    if (m_handler && m_resizeCB) (m_handler->*m_resizeCB)(size);
  }
  ...
  inline void Initialize(HANDLER *handler, CallBackKeyDown keyDownCB,
                         ...
                         CallBackMouseDown mouseDownCB,
                         CallBackMouseUp mouseUpCB,
                         CallBackMouseDraging mouseDragingCB,
                         CallBackMouseDoubleClick mouseDoubleClickCB,
                         CallBackResize resizeCB) {
    m_quit = false;
    m_keyDown = false;
    m_mouseDown = false;
    m_handler = handler;
