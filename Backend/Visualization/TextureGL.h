...
class TextureGL {
 public:
  ...
  inline void Generate(const int w, const int h,
                       const GLint filterType = GL_NEAREST) {
    if (m_texture == 0)
      Generate(filterType);
    else
      Bind();
  ...
  inline void Delete() {
    if (m_texture != 0) {
      GLint boundTexture;
      glGetIntegerv(GL_TEXTURE_BINDING_RECTANGLE_ARB, &boundTexture);
      if (boundTexture == m_texture) glBindTexture(GL_TEXTURE_RECTANGLE_ARB, 0);
  ...
  template <typename TYPE>
  inline void UploadFromCPU(const TYPE *pixels, const GLenum components) const;
  template <typename TYPE>
  inline void UploadFromCPU(const TYPE *pixels, const int w, const int h) const;
  template <typename TYPE>
  ...
  inline void UploadFromCPU(const TYPE *pixels, const int x, const int y,
                            ...
                            const GLenum components) const;
  template <typename TYPE>
  inline void DownloadToCPU(TYPE *pixels) const;
  template <typename TYPE>
  ...
  template <typename TYPE>
  inline void DownloadToCPU(TYPE *pixels, const int w, const int h,
                            const GLenum components) const;
  template <typename TYPE>
  inline void DownloadToCPU(TYPE *pixels, const int x, const int y, const int w,
                            const int h) const;
  template <typename TYPE>
  inline void DownloadToCPU(TYPE *pixels, const int x, const int y, const int w,
  ...
  inline GLenum GetAttachment() const {
    ...
    for (i = 0, attachment = GL_COLOR_ATTACHMENT0_EXT; i < 8;
      ...
      glGetFramebufferAttachmentParameterivEXT(
          GL_FRAMEBUFFER_EXT, attachment, GL_FRAMEBUFFER_ATTACHMENT_OBJECT_NAME,
          &attachedTexture);
      if (attachedTexture == m_texture) return attachment;
    }
