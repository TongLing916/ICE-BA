...
void ViewerIBA::DrawString() {
  ...
  if (KF && m_keyDrawCamTypeKF == DRAW_CAM_KF_LBA ||
    ...
    Viewer::DrawStringCurrent(
        "  fps %.2f  sync %.0f  norm %.0f  schur %.0f  cam %.0f  dep %.0f  dl "
        "%.0f",
  ...
  if (m_keyDrawViewType == DRAW_VIEW_2D) {
...
#endif
                      T.m_fileName;
    Viewer::DrawStringTopLeft(1, GLUT_BITMAP_HELVETICA_18, "%s",
                              fileName.c_str());
  } else if (m_keyDrawViewType == DRAW_VIEW_3D) {
    ...
    if (KF && C.Invalid()) {
      ...
      if (T.Valid()) {
        Viewer::DrawStringTopLeft(1, GLUT_BITMAP_HELVETICA_10,
                                  " T = %f %f %f %f", T.r00(), T.r01(), T.r02(),
                                  T.tx());
        Viewer::DrawStringTopLeft(2, GLUT_BITMAP_HELVETICA_10,
                                  "     %f %f %f %f", T.r10(), T.r11(), T.r12(),
                                  T.ty());
        Viewer::DrawStringTopLeft(3, GLUT_BITMAP_HELVETICA_10,
                                  "     %f %f %f %f", T.r20(), T.r21(), T.r22(),
                                  T.tz());
    ...
    if (m_keyDrawPrfType >= DRAW_PRF_ACCELERATION &&
      ...
      for (int i = 0, r = 0; i < Nu; ++i) {
        ...
        Viewer::DrawStringTopLeft(++r, GLUT_BITMAP_HELVETICA_10,
                                  "%s = %f %f %f  t = %f", str.c_str(), v.x(),
                                  v.y(), v.z(), u.t());
      }
        ...
        for (int iZ = 0, rowStart = 0; iZ < NZ; ++iZ) {
          ...
          if (Zp.m_iKFr != m_iKFActive) {
            continue;
                     ...
                     Zp.GetPriorMeasurement(BA_WEIGHT_FEATURE, &S, &x, NULL,
                                            &m_work, _eps)) {
            DrawString(Zp, S, x, &rowStart);
          } else {
            Viewer::DrawStringTopLeft(++rowStart, GLUT_BITMAP_HELVETICA_10,
                                      "INVALID");
          }
        }
      } else if (m_LBA->m_Zp.Pose::Valid()) {
        if (t == 0 ||
            m_LBA->m_Zp.GetPriorMeasurement(BA_WEIGHT_FEATURE, &S, &x, NULL,
                                            &m_work, _eps)) {
      ...
      if (t == 0 ||
        ...
        switch (m_keyDrawPrfType) {
          ...
          case DRAW_PRF_CAMERA_PRIOR_BIAS_ACCELERATION_MEASUREMENT:
            str = "zpba";
            v = m_LBA->m_Zp.GetBiasAccelerationMeasurement(x.Data() + ip + 3);
            break;
          case DRAW_PRF_CAMERA_PRIOR_BIAS_ACCELERATION_ERROR:
            str = "epba";
            v = m_LBA->m_Zp.GetBiasAccelerationError(C, x.Data() + ip + 3);
            break;
          ...
          case DRAW_PRF_CAMERA_PRIOR_BIAS_GYROSCOPE_MEASUREMENT:
            str = "zpbw";
            v = m_LBA->m_Zp.GetBiasGyroscopeMeasurement(x.Data() + ip + 6);
            break;
      ...
      const UT::ES<FTR::ESError, FTR::ESIndex> ES = ComputeReprojectionError(
          KF ? m_iKFActive : -1, LF ? m_iLFActive : -1);
      Viewer::DrawStringTopLeft(1, GLUT_BITMAP_HELVETICA_10,
                                "reprojection error = %f <= %f [%d] %d\n",
                                ES.Mean(), ES.Maximal(), ES.m_idxMax.m_ixFrm,
                                ES.m_idxMax.m_ix);
      ...
      if (m_keyDrawPrfType < DRAW_PRF_STATE_POSITION_ABSOLUTE) {
        Rotation3D R =
            KF ? GetCameraKF(m_iKFActive) : GetCameraLF(m_iLFActive).m_T;
        str = "r";
        ...
        if (iKFNearest == -1) {
          ...
          if (m_keyDrawPrfType >= DRAW_PRF_STATE_ROTATION_RELATIVE) {
            ...
            str = UT::String("[%d] --> [%d] ",
                             GetKeyFrame(iKFNearest)->m_T.m_iFrm, T.m_iFrm) +
                  str;
          }
#ifdef CFG_GROUND_TRUTH
  ...
  if (KF) {
    Viewer::DrawStringBottomLeft("KF = %d / %d  x = %d / %d  z = %d",
                                 m_iKFActive, nKFs, KF->m_xs.size(),
                                 m_iKF2d.back(), KF->m_zs.size());
  } else {
...
void ViewerIBA::DrawString(const std::string str, const int t, const bool r,
  ...
  if (t < 3) {
    ...
    Viewer::DrawStringTopLeft(row + 1, GLUT_BITMAP_HELVETICA_10,
                              "%s = %f %f %f", str.c_str(), v.x(), v.y(),
                              v.z());
    if (t == 2) {
      Viewer::DrawStringCurrent(GLUT_BITMAP_HELVETICA_10, " = %f",
...
#else
    Viewer::DrawStringTopLeft(row + 1, GLUT_BITMAP_HELVETICA_10,
                              "%s = %f %f %f = %f", str.c_str(), v.x(), v.y(),
                              v.z(), sqrtf(v.SquaredLength()));
    if (rowStart) {
      ++*rowStart;
    }
#endif
    ...
    if (i == -1) {
      ...
      switch (m_keyDrawPrfType) {
        ...
        case DRAW_PRF_CAMERA_PRIOR_ROTATION_MEASUREMENT:
          ...
          {
            const Rotation3D eR =
                Zp.GetReferenceRotationMeasurement(x.Data(), BA_ANGLE_EPSILON);
            v = eR.GetRodrigues(BA_ANGLE_EPSILON);
            break;
          }
        case DRAW_PRF_CAMERA_PRIOR_ROTATION_ERROR:
          str = "er";
          v = Zp.GetReferenceRotationError(C1, x.Data(), BA_ANGLE_EPSILON);
          break;
      ...
      switch (m_keyDrawPrfType) {
        ...
        case DRAW_PRF_CAMERA_PRIOR_ROTATION_MEASUREMENT:
          str = "zpr";
          {
            const Rotation3D R = Zp.GetRotationMeasurement(i, x.Data() + ip + 3,
                                                           BA_ANGLE_EPSILON);
...
void ViewerIBA::DrawTimeLine() {
  ...
  if (m_iFtrActive.Valid()) {
    ...
    for (int iFrm = nKFs, iLF = nLFs == 0 ? 0 : (m_iLF + 1) % nLFs;
         iFrm < nFrms; ++iFrm, iLF = (iLF + 1) % nLFs) {
    ...
    if (m_iFtrActive.m_iz.Valid()) {
      if (m_iFtrActive.m_iz.m_iKF != -1) {
        ...
        x = wh +
            (nKFs + (m_iFtrActive.m_iz.m_iLF + nLFs - m_iLF - 1) % nLFs) *
                ratio;
      }
      Viewer::DrawBox(x, y, sizeQuad);
    }
  ...
  if (m_keyDrawTlnType == DRAW_TLN_FEATURE_MATCH) {
    ...
    if (activeKF) {
      ...
      for (int iZ = 0; iZ < NZ; ++iZ) {
        const CameraPrior::Pose &Zp = m_GBA->m_Zps[iZ];
        if (Zp.m_iKFr == m_iKFActive) {
          Zp.GetPriorMeasurement(BA_WEIGHT_FEATURE, &S, NULL, NULL, &m_work,
                                 _eps);
...
void ViewerIBA::Get2DProjectionRange(const Point2D &x, const LA::Vector2f &Jx,
                                     const float s2, Point2D *xs) {
  ...
  for (int yr = 0, i = 0; yr < h; ++yr)
    for (int xr = 0; xr < w; ++xr, ++i) {
      ...
      if (ix1 == -1) {
        ...
        UT::ImageInterpolate(Id[iy1][ix1], Id[iy2][ix1], Id[iy1][ix2],
                             Id[iy2][ix2], RM.m_ws[i], Ir[yr][xr]);
      }
    }
}
...
void ViewerIBA::Draw3DCameras(const std::vector<float> &alphas,
                              const float alphaMin, const int iKFr) {
  glEnable(GL_BLEND);
  glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
...
void ViewerIBA::Reset3DViewPoint(const bool active) {
  ...
  m_frustrum.SetShape(z * VW_CAMERA_DEPTH_TO_SCENE_DEPTH_RATIO, xzRatio,
                      yzRatio, m_keyDrawCamSize, m_keyDrawAxisLen);
  m_frustrumActive.SetShape(
      z * VW_CAMERA_DEPTH_TO_SCENE_DEPTH_RATIO, xzRatio, yzRatio,
      m_keyDrawCamSize * VW_CAMERA_SIZE_ACTIVE_RATIO, m_keyDrawAxisLen);
  // m_translation.MakeZero();
  m_arcball.Initialize(
      m_Cv.LA::AlignedMatrix3x3f::GetTranspose(), z,
...
void ViewerIBA::UnProjectWindowPoint(const CVD::ImageRef &x, Point3D &X) {
  double Xx, Xy, Xz;
  gluUnProject(x.x, m_pWnd->size().y - x.y, 0, m_modelMatrix, m_projMatrix,
               m_viewport, &Xx, &Xy, &Xz);
...
void ViewerIBA::DrawProfile() {
  ...
  const Point2D xi(dx + whi, H - VW_STRING_BORDER_Y - VW_STRING_HEIGHT -
                                 VW_STRING_GAP_Y - hhi);
  glEnable(GL_TEXTURE_RECTANGLE_ARB);
  Update2DImageTexture();
  ...
  // Viewer::GetStringHeight(GLUT_BITMAP_HELVETICA_10) + VW_STRING_GAP_Y;
  // const float y2 = H - VW_STRING_BORDER_Y - VW_STRING_HEIGHT -
  // VW_STRING_GAP_Y;
  const float y1 =
      Viewer::GetStringYTopLeft(2, GLUT_BITMAP_HELVETICA_10) + VW_STRING_GAP_Y;
  ...
  Viewer::DrawStringAt(
      x, yb - B - Viewer::GetStringHeight(GLUT_BITMAP_HELVETICA_10),
  ...
  if (m_keyDrawPrfType >= DRAW_PRF_ACCELERATION &&
    ...
    for (int iFrm = 0; iFrm < nFrms; ++iFrm, x = ratio + x) {
      ...
      const int iKFNearest =
          (iKF != -1 ? GetKeyFrame(iKF) : GetLocalFrame(iLF))->m_iKFNearest;
      if (m_keyDrawPrfType < DRAW_PRF_STATE_POSITION_ABSOLUTE) {
        if (m_keyDrawPrfType >= DRAW_PRF_STATE_ROTATION_RELATIVE &&
            iKFNearest == -1) {
          v.Invalidate();
  ...
  if (m_keyDrawPrfType >= DRAW_PRF_CAMERA_PRIOR_ROTATION_STATE &&
    ...
    if (activeKF) {
      ...
      for (int iZ = 0; iZ < NZ; ++iZ) {
        ...
        if (Zp.m_iKFr == m_iKFActive &&
            (t == 0 ||
             Zp.GetPriorMeasurement(BA_WEIGHT_FEATURE, &S, &x, NULL, &m_work,
                                    _eps))) {
    ...
    if (t == 0 ||
        m_LBA->m_Zp.GetPriorMeasurement(BA_WEIGHT_FEATURE, &S, &_x, NULL,
                                        &m_work, _eps)) {
      const Camera C = GetCameraLF(m_LBA->m_ic2LF.front());
    ...
    if (i == -1) {
      ...
      switch (m_keyDrawPrfType) {
        case DRAW_PRF_CAMERA_PRIOR_ROTATION_STATE:
          ...
          break;
        case DRAW_PRF_CAMERA_PRIOR_ROTATION_MEASUREMENT:
          v = Zp.GetReferenceRotationMeasurement(x.Data(), BA_ANGLE_EPSILON)
                  .GetRodrigues(BA_ANGLE_EPSILON);
          break;
        case DRAW_PRF_CAMERA_PRIOR_ROTATION_ERROR:
      ...
      switch (m_keyDrawPrfType) {
        ...
        case DRAW_PRF_CAMERA_PRIOR_ROTATION_MEASUREMENT: {
          const Rotation3D R =
              Zp.GetRotationMeasurement(i, x.Data() + ip + 3, BA_ANGLE_EPSILON);
          v = R.GetRodrigues(BA_ANGLE_EPSILON);
          break;
        }
        case DRAW_PRF_CAMERA_PRIOR_ROTATION_ERROR:
...
void ViewerIBA::DrawProfileTypeStep(const int *types, const int N,
                                    const bool next) {
  const int typeBkp = m_keyDrawPrfType;
  int i =
      static_cast<int>(std::upper_bound(types, types + N, typeBkp) - types) - 1;
...
int ViewerIBA::GetDrawProfileScaleType() {
  if (m_keyDrawPrfType >= DRAW_PRF_ACCELERATION &&
    ...
    return DRAW_PRF_IMU_DELTA_BIAS_ACCELERATION_STATE +
           (m_keyDrawPrfType - DRAW_PRF_IMU_DELTA_BIAS_ACCELERATION_STATE) / 2 *
               2;
  } else if (m_keyDrawPrfType >= DRAW_PRF_CAMERA_PRIOR_ROTATION_STATE &&
             m_keyDrawPrfType <=
                 DRAW_PRF_CAMERA_PRIOR_BIAS_GYROSCOPE_COVARIANCE) {
