...
template <>
inline int Ceil<float>(const int N) {
  return SIMD_FLOAT_CEIL(N);
}
template <>
inline int Ceil<double>(const int N) {
  return SIMD_DOUBLE_CEIL(N);
}
...
inline void Set(const int N, TYPE *Vs, const TYPE v);
template <>
inline void Set<float>(const int N, float *V, const float v) {
...
template <>
inline void Add<float>(const int N, const float *Va, const float *Vb,
                       float *Vapb) {
  const int NF = SIMD_FLOAT_FLOOR(N);
...
template <>
inline void Subtract<float>(const int N, const float *Va, const float *Vb,
                            float *Vamb) {
  const int NF = SIMD_FLOAT_FLOOR(N);
  const xp128f *a = (xp128f *)Va, *b = (xp128f *)Vb;
...
template <>
inline void Subtract<double>(const int N, const double *Va, const double *Vb,
                             double *Vamb) {
  for (int i = 0; i < N; ++i) {
    Vamb[i] = Va[i] - Vb[i];
  }
...
template <>
inline void Subtract<float>(const int N, const float *Va, const float b,
                            float *Vamb) {
  const int NF = SIMD_FLOAT_FLOOR(N);
  const xp128f *a = (xp128f *)Va;
...
template <>
inline void Subtract<double>(const int N, const double *Va, const double b,
                             double *Vamb) {
  for (int i = 0; i < N; ++i) {
    Vamb[i] = Va[i] - b;
  }
...
template <>
inline void Subtract<float>(const int N, const float *Va, float *Vb) {
  const int NF = SIMD_FLOAT_FLOOR(N);
  const xp128f *a = (xp128f *)Va;
  xp128f *b = (xp128f *)Vb;
  for (int i = 0; i < NF; i += 4, ++a, ++b) {
...
template <>
inline void Multiply<float>(const int N, const float *Va, const float *Vb,
                            float *Vab) {
  const int NF = SIMD_FLOAT_FLOOR(N);
...
template <>
inline void Multiply<double>(const int N, const double *Va, const double *Vb,
                             double *Vab) {
  for (int i = 0; i < N; ++i) {
    Vab[i] = Va[i] * Vb[i];
  }
...
template <>
inline void Multiply<float>(const int N, const float *Va, float *Vb) {
  const int NF = SIMD_FLOAT_FLOOR(N);
...
inline void Multiply(const int N, const TYPE_A a, const TYPE_B *Vb,
                     TYPE_A *Vab);
inline void Multiply(const int N, const xp128f &a, const float *Vb,
                     float *Vab) {
  const int NF = SIMD_FLOAT_FLOOR(N);
  const xp128f *b = (xp128f *)Vb;
  xp128f *ab = (xp128f *)Vab;
...
template <>
inline void Multiply<float, float>(const int N, const float a, const float *Vb,
                                   float *Vab) {
  if (a == 1.0f) {
    Convert(N, Vb, Vab);
    return;
...
template <>
inline void Multiply<double, double>(const int N, const double a,
                                     const double *Vb, double *Vab) {
  if (a == 1.0) {
    Convert(N, Vb, Vab);
    return;
  }
...
template <int SKIP>
inline void MultiplyAddTo(const int N, const xp128f &a, const float *Vb,
                          float *Vab);
template <>
inline void MultiplyAddTo<0>(const int N, const xp128f &a, const float *Vb,
                             float *Vab) {
  const int NF = SIMD_FLOAT_FLOOR(N);
  const xp128f *b = (xp128f *)Vb;
  xp128f *ab = (xp128f *)Vab;
...
template <>
inline void MultiplyAddTo<1>(const int N, const xp128f &a, const float *Vb,
                             float *Vab) {
  if (N > 4) {
    Vab[1] += a[0] * Vb[1];
    Vab[2] += a[0] * Vb[2];
...
template <>
inline void MultiplyAddTo<2>(const int N, const xp128f &a, const float *Vb,
                             float *Vab) {
  if (N > 4) {
    Vab[2] += a[0] * Vb[2];
    Vab[3] += a[0] * Vb[3];
...
template <>
inline void MultiplyAddTo<3>(const int N, const xp128f &a, const float *Vb,
                             float *Vab) {
  if (N > 4) {
    Vab[3] += a[0] * Vb[3];
    MultiplyAddTo<0>(N - 4, a, Vb + 4, Vab + 4);
...
inline void MultiplyAddTo(const int i, const int N, const xp128f &a,
                          const float *Vb, float *Vab) {
  switch (i) {
    case 0:
...
inline void MultiplyAddTo(const int N, const float a, const float *Vb,
                          float *Vab) {
  const xp128f _a = xp128f::get(a);
  MultiplyAddTo(0, N, _a, Vb, Vab);
}
inline void MultiplyAddTo(const int i, const int N, const float a,
                          const float *Vb, float *Vab) {
  const xp128f _a = xp128f::get(a);
...
template <typename TYPE>
inline void MultiplyAddTo(const int N, const TYPE *Va, const TYPE *Vb,
                          TYPE *Vab);
template <>
inline void MultiplyAddTo<float>(const int N, const float *Va, const float *Vb,
                                 float *Vab) {
  const int NF = SIMD_FLOAT_FLOOR(N);
  const xp128f *a = (const xp128f *)Va, *b = (xp128f *)Vb;
  xp128f *ab = (xp128f *)Vab;
...
template <>
inline void Minus<float>(const int N, float *V) {
  Minus<0>(N, V);
}
template <>
