...
class CandidateVector {
 public:
  ...
  inline void MakeZero() {
    ...
    for (int i = 0; i < N; ++i) m_data[i].Set(i, 0);
  }
  inline void RemoveZero() {
    int i, j;
    const int N = int(m_data.size());
    for (i = j = 0; i < N; ++i) {
      if (m_data[i].m_score != 0) m_data[j++] = m_data[i];
    }
    m_data.resize(j);
  }
  ...
  inline void MarkFirstN(const int N, std::vector<ubyte> &marks,
                         const ubyte mark) const {
    const int kN = std::min(Size(), N);
    for (int i = 0; i < kN; ++i) marks[m_data[i].m_idx] = mark;
  }
  inline void GetFirstN(const int N, std::vector<int> &idxs) const {
    const int kN = std::min(Size(), N);
    idxs.resize(kN);
    for (int i = 0; i < kN; ++i) idxs[i] = m_data[i].m_idx;
  }
