...
template <>
inline ubyte Random<ubyte>() {
  return Random<int>() & 255;
}
template <>
inline ushort Random<ushort>() {
  return ushort(Random<int>());
}
template <>
...
template <>
inline float EpsilonMin<float>() {
  return UT_FLT_EPSILON_MIN;
}
template <>
inline double EpsilonMin<double>() {
  return UT_DBL_EPSILON_MIN;
}
template <typename TYPE>
inline TYPE EpsilonMax();
template <>
inline float EpsilonMax<float>() {
  return UT_FLT_EPSILON_MAX;
}
template <>
inline double EpsilonMax<double>() {
  return UT_DBL_EPSILON_MAX;
}
template <typename TYPE>
...
inline TYPE Invalid();
template <>
inline ubyte Invalid<ubyte>() {
  return UCHAR_MAX;
}
template <>
inline int16_t Invalid<int16_t>() {
  return SHRT_MAX;
}
template <>
inline ushort Invalid<ushort>() {
  return USHRT_MAX;
}
template <>
inline int Invalid<int>() {
  return INT_MAX;
}
template <>
inline float Invalid<float>() {
  return FLT_MAX;
}
template <>
inline double Invalid<double>() {
  return DBL_MAX;
}
template <typename TYPE>
...
void SaveHistogram(const std::string fileName, const std::vector<float> &vs,
                   ...
                   const float vMax = FLT_MAX);

template <typename TYPE>
inline bool Equal(const TYPE v1, const TYPE v2, const TYPE epsAbs = 0,
                  const TYPE epsRel = 0) {
...
template <class TYPE>
inline int VectorSearchFirstFlagNot(const std::vector<TYPE> &V, const int i1,
...
inline void ImageInterpolate(const TYPE_SRC i11, const TYPE_SRC i12,
                             const TYPE_SRC i21, const TYPE_SRC i22,
                             const xp128f &w, TYPE_DST &ip);
template <>
...
inline void ImageInterpolate<int16_t, float>(const int16_t i11,
                                             const int16_t i12,
                                             const int16_t i21,
                                             const int16_t i22, const xp128f &w,
                                             float &ip) {
  xp128f tmp;
  tmp.vset_all_lane(static_cast<float>(i11), static_cast<float>(i12),
                    static_cast<float>(i21), static_cast<float>(i22));
...
inline bool AssertReduction<float>(const float &v1, const float &v2,
                                   const int verbose, const std::string str,
