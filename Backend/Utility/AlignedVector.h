...
class AlignedVector {
 public:
  ...
  inline TYPE &Push() {
    if (m_N == m_capacity) {
      const int growth =
          GROWTH == 0 ? std::max(std::min(m_capacity, GROWTH_MAX), 1) : GROWTH;
      m_capacity += growth;
      TYPE *dataBkp = m_data;
  ...
  inline void Insert(const int i, const AlignedVector<TYPE> &V,
  ...
  inline AlignedVector<TYPE, GROWTH, GROWTH_MAX> GetBlock(const int i,
                                                          const int N) {
#ifdef CFG_DEBUG
