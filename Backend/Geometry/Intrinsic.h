...
class Intrinsic {
 public:
  class Parameter {
   public:
    ...
    inline void ImageToNormalized(const float S[2][2],
                                  Point2DCovariance &Sn) const {
      Sn.sxx() = S[0][0] * m_fxxI;
      Sn.sxy() = S[0][1] * m_fxyI;
      Sn.syy() = S[1][1] * m_fyyI;
    ...
    inline void AssertConsistency(const bool fishEye, const bool radial6,
                                  const bool tangential) const {
      ...
      for (int yr = 0, i = 0; yr < h; ++yr) {
        ...
        for (int xr = 0; xr < w; ++xr, ++i) {
          ...
          if (fishEye) {
            ...
            const float s = t1 * (k.m_ds[3] * t8 + k.m_ds[2] * t6 +
                                  k.m_ds[1] * t4 + k.m_ds[0] * t2 + 1.0f) /
                            r;
            xd.x() = s * x;
            xd.y() = s * y;
          } else {
 ...
 public:
  ...
  inline void Set(const int w, const int h, const float fx,
                  ...
                  const float fyr = 0.0f, const float cxr = 0.0f,
    ...
    m_k.Set(fx, fy == 0.0f ? fx : fy, cx == 0.0f ? _cxr : cx,
            cy == 0.0f ? _cyr : cy);
...
//#if 1
  ...
  static inline void NormalizedToImage(const Intrinsic &K1, const Intrinsic &K2,
    ...
    H.m_20_21_22_r2() = M.m_20_21_22_r2() - (M.m_00_01_02_r0() * K1.FxIcx() +
                                             M.m_10_11_12_r1() * K1.FyIcy());
    H.Transpose();
  }
  ...
  class UndistortionMap {
   public:
    ...
    inline Point2D Get(const Point2D &xd) const {
      ...
      UT::ImageInterpolate<float, float>(xn11.x(), xn12.x(), xn21.x(), xn22.x(),
                                         w, xn.x());
      UT::ImageInterpolate<float, float>(xn11.y(), xn12.y(), xn21.y(), xn22.y(),
                                         w, xn.y());
      return xn;
    }
  ...
  inline void Distort(const Point2D &xn, Point2D *xd) const {
    ...
    if (m_fishEye) {
      ...
      const float t4 = t2 * t2, t6 = t2 * t4, t8 = t4 * t4;
      const float s =
          t1 * (ds[3] * t8 + ds[2] * t6 + ds[1] * t4 + ds[0] * t2 + 1.0f) / r;
      xd->x() = s * x;
      xd->y() = s * y;
    } else {
      const float r4 = r2 * r2, r6 = r2 * r4;
      float dr = ds[4] * r6 + ds[1] * r4 + ds[0] * r2 + 1.0f;
      if (m_radial6) dr /= ds[7] * r6 + ds[6] * r4 + ds[5] * r2 + 1.0f;
      xd->x() = dr * x;
      xd->y() = dr * y;
      if (m_tangential) {
        const float xy = x * y;
