...
class Rigid3D : public Rotation3D {
 ...
 public:
  ...
  inline void operator=(const Rigid3D &T) { memcpy(this, &T, sizeof(Rigid3D)); }
  ...
  inline void Set(const double *T) {
    r00_r01_r02_tx().vset_all_lane(float(T[0]), float(T[1]), float(T[2]),
                                   float(T[3]));
    r10_r11_r12_ty().vset_all_lane(float(T[4]), float(T[5]), float(T[6]),
                                   float(T[7]));
    r20_r21_r22_tz().vset_all_lane(float(T[8]), float(T[9]), float(T[10]),
                                   float(T[11]));
  }
  ...
  inline void ApplyRotationInversely(const LA::AlignedVector3f &X,
  ...
  inline bool AssertEqual(const Rigid3D &T, const int verbose = 1,
    ...
    if (Rotation3D::AssertEqual(T, verbose, str + ".m_R", rEps) &&
        GetTranslation().AssertEqual(T.GetTranslation(), verbose, str + ".m_t",
                                     tEps)) {
      return true;
    } else if (verbose) {
      UT::PrintSeparator();
  ...
  static inline void ABI(const Rigid3D &Ta, const Rigid3D &Tb, Rigid3D &TabI) {
    ...
    TabI.MakeOrthogonal();
    TabI.SetTranslation(Ta.GetTranslation() -
                        TabI.GetAppliedRotation(Tb.GetTranslation()));
  }
  static inline void ABI(const Row &Ta, const Rigid3D &Tb, Row &TabI) {
    LA::AlignedMatrix3x3f::aBT(Ta, Tb, TabI);
    TabI.t() =
        Ta.t() - (TabI.r0_r1_r2_t() * Tb.GetTranslation().v012r()).vsum_012();
  }
};
#endif  // _RIGID_H_
