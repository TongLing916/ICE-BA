...
template <int FUNCTION>
inline float Cost(const float r2);
template <int FUNCTION>
inline float Cost(const AlignedVector<float> &r2s);
template <int FUNCTION>
...
template <int FUNCTION>
inline int Count(const AlignedVector<float> &r2s) {
  int SN = 0;
...
template <>
inline float ChiSquareDistance<3>(const float p, const float w) {
#ifdef CFG_DEBUG
...
template <>
inline float Weight<FUNCTION_NONE>(const float r2) {
  return 1.0f;
}
template <>
...
template <>
inline float Cost<FUNCTION_NONE>(const float r2) {
  return r2;
}
template <>
inline float Cost<FUNCTION_NONE>(const AlignedVector<float> &r2s) {
  LA::AlignedVectorXf _r2s((float *)r2s.Data(), r2s.Size(), false);
  return _r2s.Sum();
}

template <>
inline float Weight<FUNCTION_HUBER>(const float r2) {
...
template <>
inline float Cost<FUNCTION_HUBER>(const float r2) {
  // if (r2 > ME_VARIANCE_HUBER) {
...
template <>
inline float Cost<FUNCTION_HUBER>(const AlignedVector<float> &r2s) {
  float C = 0.0f;
  const int N = r2s.Size();
  for (int i = 0; i < N; ++i) {
...
template <>
inline float Variance<FUNCTION_HUBER>() {
  return ME_VARIANCE_HUBER;
}

template <>
inline float Weight<FUNCTION_TUKEY>(const float r2) {
...
template <>
inline float Cost<FUNCTION_TUKEY>(const float r2) {
  // if (r2 > ME_VARIANCE_TUKEY) {
...
template <>
inline float Cost<FUNCTION_TUKEY>(const AlignedVector<float> &r2s) {
...
template <>
inline float Variance<FUNCTION_TUKEY>() {
  return ME_VARIANCE_TUKEY;
}

template <int DOF>
inline float WeightStudentT(const float r2) {
...
template <int DOF>
inline float CostStudentT(const float r2) {
  return logf(r2 / (ME_VARIANCE_STUDENT_T * DOF) + 1.0f);
}
template <int DOF>
inline float CostStudentT(const AlignedVector<float> &r2s) {
  float C = 0.0f;
...
template <>
inline float Weight<FUNCTION_STUDENT_T_5>(const float r2) {
  return WeightStudentT<5>(r2);
}
template <>
inline float Weight<FUNCTION_CAUCHY>(const float r2) {
...
template <>
inline float Cost<FUNCTION_STUDENT_T_5>(const float r2) {
  return CostStudentT<5>(r2);
}
template <>
inline float Cost<FUNCTION_CAUCHY>(const float r2) {
...
template <>
inline float Cost<FUNCTION_CAUCHY>(const AlignedVector<float> &r2s) {
