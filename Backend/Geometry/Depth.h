...
class InverseGaussian {
 public:
  ...
  inline InverseGaussian(const float u)
      : m_u(u), m_s2(DEPTH_VARIANCE_INITIAL) {}
  ...
  static inline void Project(const LA::AlignedVector3f &t12, const Point2D &x1,
  ...
  static inline void Project(const LA::AlignedVector3f &t12,
                             const LA::Vector3f &Rx1, const float d1,
                             LA::Vector2f &x2) {
    const LA::AlignedVector3f t = t12 * d1;
  ...
  inline bool Project(const Rigid3D &T12, const Point2D &x1,
                      const BoundingBox2D &B2, Point2D &x2,
                      LA::Vector2f &Jx2) const {
    const xp128f t = xp128f::get(x1.x(), x1.y(), 1.0f, u());
    const float z12 = (T12.r20_r21_r22_tz() * t).vsum_all();
    if (z12 < FLT_EPSILON) {
  ...
  inline Point2D GetProjected(const LA::AlignedVector3f &t12,
  ...
  static inline void ProjectD(const Rigid3D &T21, const Point2D &x2,
                              const float d2, float &d1, LA::Vector2f &Jx2,
                              float &Jd2, LA::AlignedVector6f &Jpr21) {
...
class InverseGaussianBeta : public InverseGaussian {
 public:
  ...
  inline bool Update(const InverseGaussian &d) {
    ...
    const float SabI = 1.0f / (a() + b()), Ss2I = 1.0f / Ss2, dz = d.u() - u(),
                dz2 = dz * dz;
    const float C1 =
  ...
  inline void Print(const std::string str = "", const bool e = false,
    ...
    UT::Print("  pi = %.2f%% (%.2f %.2f)", UT::Percentage(a(), a() + b()), a(),
              b());
    if (n) {
      UT::Print("\n");
    }
