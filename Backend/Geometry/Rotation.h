...
class AxisAngle : public LA::AlignedVector4f {
 public:
  ...
  inline bool AssertEqual(const AxisAngle &kth, const int verbose = 1,
    ...
    if (UT::AssertZero(UT_DOT_TO_ANGLE(d), verbose, str + ".dot",
...
class Quaternion : public LA::AlignedVector4f {
 public:
  ...
  inline void Slerp(const Quaternion &q1, const Quaternion &q2, const float t1,
                    const float t2, const float t) {
...
class SkewSymmetricMatrix : public LA::AlignedVector3f {
 public:
  ...
  static inline void GetTranspose(const LA::AlignedVector3f &v,
                                  LA::AlignedMatrix3x3f &M) {
    M.m00() = 0.0f;
    M.m01() = v.z();
    M.m02() = -v.y();
...
class Rotation3D : public LA::AlignedMatrix3x3f {
 public:
  ...
  static inline void GetRodriguesJacobianInverse(const LA::AlignedVector3f &w,
    ...
    const float t2 = th2I - (1.0f + UT_COSF(th)) / (2.0f * th * UT_SINF(th)),
                t3 = 1.0f - t2 * th2;
    const LA::AlignedVector3f t1w(w.xyzr() * t1);
    const LA::AlignedVector3f t2w2(w2.xyzr() * t2);
    const float t2wx = t2 * w.x();
  ...
  inline void Apply(const Point2D &x, Point2D &Rx,
                    LA::AlignedMatrix2x2f &Jx) const {
  ...
  inline bool AssertEqual(const Rotation3D &R, const int verbose = 1,
    ...
    // const float eps = 0.1f * UT_FACTOR_DEG_TO_RAD;
    if ((eps < 0.0f &&
         UT::VectorAssertEqual(&r00(), &R.r00(), 12, verbose, str, eps)) ||
  ...
  inline bool AssertOrthogonal(const int verbose = 1,
                               const std::string str = "") const {
  ...
  inline bool AssertOrthogonalRows(const int verbose = 1,
    ...
    if (!UT::AssertEqual(UT_DOT_TO_ANGLE(d00), 0.0f, verbose, str + ".rxTrx",
                         eps)) {
      return false;
    }
    const float d01 = r_00_01_02_x().vdot012(r_10_11_12_x());
    if (!UT::AssertEqual(UT_DOT_TO_ANGLE(d01) - UT_PI_2, 0.0f, verbose,
                         str + ".rxTry", eps)) {
      return false;
    }
    const float d02 = r_00_01_02_x().vdot012(r_20_21_22_x());
    if (!UT::AssertEqual(UT_DOT_TO_ANGLE(d02) - UT_PI_2, 0.0f, verbose,
                         str + ".rxTrz", eps)) {
      return false;
    }
    const float d11 = r_10_11_12_x().vdot012(r_10_11_12_x());
    if (!UT::AssertEqual(UT_DOT_TO_ANGLE(d11), 0.0f, verbose, str + ".ryTry",
                         eps)) {
      return false;
    }
    const float d12 = r_10_11_12_x().vdot012(r_20_21_22_x());
    if (!UT::AssertEqual(UT_DOT_TO_ANGLE(d12) - UT_PI_2, 0.0f, verbose,
                         str + ".ryTrz", eps)) {
      return false;
    }
    const float d22 = r_20_21_22_x().vdot012(r_20_21_22_x());
    if (!UT::AssertEqual(UT_DOT_TO_ANGLE(d22), 0.0f, verbose, str + ".rzTrz",
                         eps)) {
      return false;
    }
    ...
    if (!UT::AssertEqual(UT_DOT_TO_ANGLE(dc0), 0.0f, verbose, str + ".rx",
                         eps)) {
      return false;
    }
    c = r_20_21_22_x().vcross012(r_00_01_02_x());
    const float dc1 = c.vdot012(r_10_11_12_x());
    if (!UT::AssertEqual(UT_DOT_TO_ANGLE(dc1), 0.0f, verbose, str + ".ry",
                         eps)) {
      return false;
    }
    c = r_00_01_02_x().vcross012(r_10_11_12_x());
    const float dc2 = c.vdot012(r_20_21_22_x());
    if (!UT::AssertEqual(UT_DOT_TO_ANGLE(dc2), 0.0f, verbose, str + ".rz",
                         eps)) {
      return false;
    }
    return true;
  ...
  static inline void AB(const Rotation3D &Ra, const Rotation3D &Rb,
                        Rotation3D &Rab) {
    const Rotation3D RbT = Rb.GetTranspose();
    ABT(Ra, RbT, Rab);
  }
  static inline void ABT(const Rotation3D &Ra, const Rotation3D &Rb,
                         Rotation3D &RaRbT) {
    LA::AlignedMatrix3x3f::ABT(Ra, Rb, RaRbT);
    RaRbT.MakeOrthogonal();
  }
  static inline void ATB(const Rotation3D &Ra, const Rotation3D &Rb,
                         Rotation3D &RaTRb) {
  ...
  static inline EigenMatrix3x3f GetRodriguesJacobianInverse(
    ...
    if (th < eps) {
      ...
      return EigenMatrix3x3f(
          ...
          (1.0f / (th * th) - (1.0f + cosf(th)) / (2.0f * th * sinf(th))) *
              e_S * e_S);
    }
  }
