...
void LocalBundleAdjustor::PushDeleteMapPoints(const int iFrm,
...
void LocalBundleAdjustor::PushUpdateCameras(
...
//#if 1
  ...
  if (m_verbose >= 2) {
    ...
    if (g_viewer) {
      ...
      g_viewer->ActivateFrame(g_viewer->GetKeyFrames() +
                              g_viewer->GetLocalFrames() - 1);
    }
  }
#endif
#if 0
...
//#ifdef CFG_DEBUG
    ...
    if (m_verbose) {
      ...
      PrintErrorStatistic(UT::String("*%2d: ", m_iIter), m_CsLF, m_CsKF, m_ds,
                          m_DsLF, m_verbose >= 2);
    }
#endif
#ifdef LBA_DEBUG_VIEW
...
//}
    ...
    for (m_iIterDL = 0; m_iIterDL < nItersDL; ++m_iIterDL) {
      ...
      if (m_verbose) {
        ...
        PrintErrorStatistic(str, m_CsLFBkp, m_CsKF, m_dsBkp, m_xsDL,
                            m_verbose >= 2);
      }
#endif
      ...
      if (m_verbose) {
        if (m_verbose >= 2) {
          UT::Print(" %f * (%e %e) <= %e %f\n", m_beta, sqrtf(m_x2GN),
                    sqrtf(m_x2GD), sqrtf(m_delta2), m_rho);
        } else {
          UT::Print(" %.3f %.2e <= %.2e %.1f\n", m_beta, sqrtf(m_x2DL),
                    sqrtf(m_delta2), m_rho);
        }
      }
#endif
#ifdef LBA_DEBUG_EIGEN
...
//#ifdef CFG_DEBUG
  ...
  if (m_history >= 1 && (m_hists.empty() || T.m_iFrm > m_hists.back().m_iFrm)) {
    ...
    if (m_history >= 2) {
      ...
      hist.m_ESp =
          ComputeErrorStatistic(m_CsLFBkp, m_CsKF, m_dsBkp, m_xsDL, false);
#ifdef CFG_GROUND_TRUTH
      if (m_CsGT) {
        SolveSchurComplementGT(m_CsLFBkp, &m_xsGT);
        if (m_dsGT) {
          if (m_history >= 3) {
            EmbeddedPointIteration(m_CsLFGT, m_CsKFGT, m_ucsKFGT, m_udsGT,
                                   m_dsGT);
          }
          SolveBackSubstitutionGT(m_dsBkp, &m_xsGT);
          hist.m_ESaGT =
              ComputeErrorStatistic(m_CsLFGT, m_CsKFGT, *m_dsGT, m_DsLFGT);
          hist.m_ESpGT = ComputeErrorStatistic(m_CsLFBkp, m_CsKFBkp, m_dsBkp,
                                               m_xsGT, false);
        }
      }
#endif
      ...
      if (m_CsGT) {
        ...
        const float epsba = UT::Inverse(BA_VARIANCE_MAX_BIAS_ACCELERATION,
                                        BA_WEIGHT_FEATURE, eps);
        const float epsbw =
            UT::Inverse(BA_VARIANCE_MAX_BIAS_GYROSCOPE, BA_WEIGHT_FEATURE, eps);
        const float _eps[] = {epsp,  epsp,  epsp,  epsr,  epsr,
                              epsr,  epsv,  epsv,  epsv,  epsba,
                              epsba, epsba, epsbw, epsbw, epsbw};
        LA::AlignedMatrixXf S;
        LA::AlignedVectorXf x;
        float xTb;
        if (m_ZpKF.Valid()) {
          if (m_ZpKF.GetPriorMeasurement(BA_WEIGHT_FEATURE, &S, &x, &xTb,
                                         &m_work, _eps)) {
        ...
        if (m_Zp.Pose::Valid()) {
          if (m_Zp.GetPriorMotion(&m_ZpLF, &m_work, _eps) &&
              m_ZpLF.GetPriorMeasurement(BA_WEIGHT_FEATURE, &S, &x, &xTb,
                                         &m_work, _eps + 6)) {
...
#endif
          if (m_Zp.GetPriorMeasurement(BA_WEIGHT_FEATURE, &S, &x, &xTb, &m_work,
                                       _eps)) {
    ...
    for (int i = 0; i < 2; ++i) {
      if (i == 0) {
        ...
        const int iKFNearest =
            LF.m_iKFNearest == -1 ? LF.m_iKFsMatch.front() : LF.m_iKFNearest;
        UT::Print("[%d] <-- [%d]\n", LF.m_T.m_iFrm,
                  m_KFs[iKFNearest].m_T.m_iFrm);
        Cc = m_CsLF[iLF];
        Ck.MakeIdentity();
        const int NkKF = static_cast<int>(LF.m_iKFsMatch.size());
        for (int ik = 0; ik < NkKF; ++ik) Ck = m_CsKF[LF.m_iKFsMatch[ik]] * Ck;
...
bool LocalBundleAdjustor::SaveCameras(const std::string fileName,
                                      const bool poseOnly) {
  FILE *fp = fopen(fileName.c_str(), "w");
  if (!fp) {
    return false;
  ...
  for (int i = 0; i < N; ++i) {
    ...
    R.GetQuaternion(q);
    fprintf(fp, "%f %f %f %f %f %f %f %f", hist.m_t, p.x(), p.y(), p.z(), q.x(),
            q.y(), q.z(), q.w());
    if (!poseOnly) {
      ...
      fprintf(fp, " %f %f %f %f %f %f %f %f %f", v.x(), v.y(), v.z(), ba.x(),
              ba.y(), ba.z(), bw.x(), bw.y(), bw.z());
    }
    fprintf(fp, "\n");
  }
#endif
...
bool LocalBundleAdjustor::SaveCosts(const std::string fileName,
                                    const int type) {
  FILE *fp = fopen(fileName.c_str(), "w");
  if (!fp) {
    return false;
...
bool LocalBundleAdjustor::SaveResiduals(const std::string fileName,
                                        const int type) {
  FILE *fp = fopen(fileName.c_str(), "w");
  if (!fp) {
    return false;
...
bool LocalBundleAdjustor::SavePriors(const std::string fileName,
                                     const int type) {
  FILE *fp = fopen(fileName.c_str(), "w");
  if (!fp) {
    return false;
...
bool LocalBundleAdjustor::SaveMarginalizations(const std::string fileName,
                                               const int type) {
  FILE *fp = fopen(fileName.c_str(), "w");
  if (!fp) {
    return false;
...
void LocalBundleAdjustor::ComputeErrorFeature(
    const FRM::Frame *F, const Rigid3D &C, const AlignedVector<Rigid3D> &CsKF,
  ...
  for (std::list<InputLocalFrame>::iterator ILF = m_ILFs2.begin();
    ...
    if (m_LFs.empty()) {
      ...
      if (LBA_PROPAGATE_CAMERA) {
        IMU::Delta D;
        IMU::PreIntegrate(ILF->m_us, _t, ILF->m_T.m_t, _C, &D, &m_work, false,
                          _u, NULL, BA_ANGLE_EPSILON);
    ...
    for (std::list<InputType>::const_iterator IT = m_ITs2.begin();
      ...
      if (m_CsGT && m_dsGT) {
        ...
        for (int iX1 = 0, iX2 = 0; iX1 < NX; iX1 = iX2) {
          ...
          const int Nx =
              iKF < m_CsKFGT.Size() ? m_iKF2d[iKF + 1] - m_iKF2d[iKF] : 0;
    ...
    if (IT == IT_LOCAL_FRAME) {
      ...
      if (ILF.m_d.Invalid()) {
        ...
        for (std::list<GlobalMap::InputKeyFrame>::iterator IKF =
          ...
          const int NX = static_cast<int>(IKF->m_Xs.size()), N = NX + Nz,
                    NC = SIMD_FLOAT_CEIL(N);
          m_work.Resize(NC * 3);
...
#endif
        if (X.m_d.Invalid()) {
          X.m_d.Initialize(X.m_iKF == nKFs ? IKF.m_d.u()
                                           : m_KFs[X.m_iKF].m_d.u());
        }
      }
      _PushKeyFrame(IKF);
...
void LocalBundleAdjustor::UpdateData() {
  ...
  for (int iLF = 0; iLF < nLFs; ++iLF) {
    if (m_UcsLF[iLF] & LM_FLAG_FRAME_UPDATE_CAMERA_LF) {
      ComputeErrorFeature(&m_LFs[iLF], m_CsLF[iLF].m_T, m_CsKF, m_ds,
                          &m_esLF[iLF].second);
    }
  }
  const int nKFs = static_cast<int>(m_KFs.size());
  ...
  for (int iKF = 0; iKF < nKFs; ++iKF) {
    if (m_UcsKF[iKF] & LM_FLAG_FRAME_UPDATE_CAMERA_KF) {
      ComputeErrorFeature(&m_KFs[iKF], m_CsKF[iKF], m_CsKF, m_ds,
                          &m_esKF[iKF].second, iKF);
    }
  }
#endif
  m_LM->LBA_Update(iFrm1, iFrm2, m_ic2LF, m_CsLF, m_UcsLF, m_iFrmsKF, m_CsKF,
                   m_UcsKF, m_iKF2d, m_ds, m_Uds
#ifdef CFG_CHECK_REPROJECTION
                   ,
                   m_esLF, m_esKF
#endif
...
static inline void PrintReduction(const IMU::Delta::Error &e1,
                                  const IMU::Delta::Error *e2) {
  ...
  UT::PrintReduction(sqrtf(e1.m_er.SquaredLength()) * UT_FACTOR_RAD_TO_DEG, er,
                     3, "  r = ");
...
static inline void PrintReduction(const Camera::Calibration &K, const int ix,
  ...
  if (e1.Valid()) {
    ...
    UT::PrintReduction(sqrtf(e1.SquaredLength() * K.m_K.fxy()), e, 3, "", false,
                       e1r.Invalid());
  }
  if (e1r.Valid()) {
    for (int i = 0; i < 3; ++i) {
    ...
    UT::PrintReduction(
        sqrtf(e1r.SquaredLength() * K.m_Kr.fxy()), e, 3,
  ...
  UT::PrintReduction(sqrtf(e1.m_ex.SquaredLength() * K.m_K.fxy()), e, 3,
                     UT::String("  %d ", ix));
#endif
}
static inline void PrintDifference(const LA::Vector2f *xg, const int i1 = 0,
                                   const int i2 = 1) {
  UT::Print(" eg = %f\n",
            sqrtf((xg[i1] - xg[i2]).SquaredLength()) * UT_FACTOR_RAD_TO_DEG);
}
static inline void PrintDifference(const CameraPrior::Element::EC *xc,
...
static inline void PrintDifference(
  ...
  for (int i = 0; i < N; ++i) {
    UT::Print(" %f",
              sqrtf((xks[i1][i].m_ep - xks[i2][i].m_ep).SquaredLength()));
  }
  UT::Print("\n");
  UT::Print(" er =");
  for (int i = 0; i < N; ++i) {
...
static inline void PrintDifference(
    const LA::Vector2f *xg, const std::vector<CameraPrior::Element::EC> *xks,
...
//#ifdef CFG_DEBUG
  ...
  const float epsbw =
      UT::Inverse(BA_VARIANCE_MAX_BIAS_GYROSCOPE, BA_WEIGHT_FEATURE, eps);
  const float _eps[] = {epsp, epsp,  epsp,  epsr,  epsr,  epsr,  epsv, epsv,
                        epsv, epsba, epsba, epsba, epsbw, epsbw, epsbw};
...
//#ifdef LBA_DEBUG_GROUND_TRUTH_MEASUREMENT
        ...
        // m_ZpLF.MakeZero();
        m_ZpLF.Initialize(
            BA_WEIGHT_PRIOR_CAMERA_INITIAL, BA_VARIANCE_PRIOR_VELOCITY_RESET,
      ...
      if ((m_Zp.GetPriorPose(iKFr, &m_ZpKF, &m_work, _eps) ||
           !LBA_MARGINALIZATION_CHECK_INVERTIBLE) &&
          m_ZpKF.MarginalizeUninformative(
              BA_WEIGHT_FEATURE, BA_VARIANCE_PRIOR_POSITION_INFORMATIVE,
              BA_VARIANCE_PRIOR_ROTATION_INFORMATIVE, &m_idxsTmp1, &m_work,
              _eps)) {
    ...
    // m_ZpLF.m_Amm *= sp;
    // m_ZpLF.m_bm *= sp;
    m_Zp.Initialize(BA_WEIGHT_PRIOR_CAMERA_INITIAL /* * sp*/, iKFr, Tr, s2g,
                    m_ZpLF, true);
#ifdef LBA_DEBUG_GROUND_TRUTH_MEASUREMENT
    if (m_CsGT) {
    ...
    if (m_history >= 2) {
      m_MH.Initialize(
          BA_WEIGHT_PRIOR_CAMERA_INITIAL, s2g, BA_VARIANCE_PRIOR_POSITION_NEW,
          BA_VARIANCE_PRIOR_ROTATION_NEW, BA_VARIANCE_PRIOR_VELOCITY_NEW,
          BA_VARIANCE_PRIOR_BIAS_ACCELERATION_NEW,
          BA_VARIANCE_PRIOR_BIAS_GYROSCOPE_NEW, m_ZpLF, &m_work, _eps);
#ifdef CFG_GROUND_TRUTH
      if (m_CsGT) {
    ...
    D.GetFactor(BA_WEIGHT_IMU /* * sd*/, C1, C2, m_K.m_pu, &e, &J, &A,
                BA_ANGLE_EPSILON);
#ifdef LBA_DEBUG_PRINT_MARGINALIZATION
    LA::Vector2f xg[3];
...
#ifdef CFG_GROUND_TRUTH
    if (m_CsGT) {
      m_Zp.GetError(m_CsKFGT[iKFr], m_CsLFGT[iLF1], &xg[0], NULL, &xm1[0],
                    BA_ANGLE_EPSILON);
    } else
#endif
    {
...
#ifdef LBA_DEBUG_GROUND_TRUTH_MEASUREMENT
      if (m_CsGT) {
        m_Zp.Pose::DebugSetMeasurement(m_CsKFGT[iKFr], false,
                                       &m_CsLFGT[iLF2].m_T);
      }
#endif
#ifdef CFG_HISTORY
...
//#ifdef CFG_DEBUG
...
#ifdef CFG_GROUND_TRUTH
    if (m_CsGT) {
      m_Zp.GetError(m_CsKFGT[iKFr], m_CsLFGT[iLF2], NULL, &xc2[0], &xm2[0],
                    BA_ANGLE_EPSILON);
    }
#endif
#ifdef CFG_HISTORY
    ...
    for (int i = 0; i < 3; ++i) {
      if (xg[i].Valid()) {
        _e[i] = e;
        D.GetError(J, xg[i], xm1[i].m_ev, xm1[i].m_eba, xm1[i].m_ebw,
                   xc2[i].m_ep, xc2[i].m_er, xm2[i].m_ev, xm2[i].m_eba,
                   xm2[i].m_ebw, _e[i]);
      } else {
        _e[i].Invalidate();
      }
    ...
    const float s2d = UT::Inverse(BA_VARIANCE_PRIOR_DEPTH_NEW,
    ...
    // BA_WEIGHT_FEATURE;
    // const float s2rMax = BA_VARIANCE_PRIOR_ROTATION_INSERT_MAX /
    // BA_WEIGHT_FEATURE;
      ...
      if (iKF == m_Zp.m_iKFr) {
        *Tk1 = Tr1;
        ...
        for (int iz = Z.m_iz1; iz < Z.m_iz2; ++iz) {
          ...
          const int ix = z.m_ix;
          FTR::GetFactor<LBA_ME_FUNCTION>(BA_WEIGHT_FEATURE, Tk1, KF.m_xs[ix],
        ...
        const std::vector<int>::const_iterator it =
            std::lower_bound(m_Zp.m_iKFs.begin(), m_Zp.m_iKFs.end(), iKF);
...
#ifdef CFG_GROUND_TRUTH
            if (m_CsGT) {
              m_MH.InsertGT(m_Zp, m_CsKFGT[m_Zp.m_iKFr], m_CsKFGT[iKF],
                            &m_work);
            }
#endif
          }
...
//#ifdef CFG_DEBUG
      ...
      if (m_Zp.GetPriorMeasurement(BA_WEIGHT_FEATURE, &S, &x, NULL, &m_work,
                                   _eps)) {
...
//#ifdef LBA_DEBUG_PRINT_MARGINALIZATION
      ...
      if (m_history >= 2) {
        m_MH.Initialize(
            BA_WEIGHT_PRIOR_CAMERA_INITIAL, BA_VARIANCE_PRIOR_GRAVITY_NEW,
    ...
    const IMU::Delta &D = m_DsLF[iLF2];
    D.GetFactor(BA_WEIGHT_IMU /* * sd*/, C1, C2, m_K.m_pu, _Tr, &e, &J, &A,
                BA_ANGLE_EPSILON);
#ifdef LBA_DEBUG_PRINT_MARGINALIZATION
    LA::Vector2f xg[3];
...
#ifdef CFG_GROUND_TRUTH
    if (m_CsGT) {
      m_Zp.GetError(m_CsKFGT[iKFr], m_CsLFGT[iLF1], &xg[0], &xc1[0], &xm1[0],
                    BA_ANGLE_EPSILON);
      const int Nk = static_cast<int>(m_Zp.m_iKFs.size() - 1);
      xks[0].resize(Nk);
      for (int i = 0; i < Nk; ++i) {
        m_Zp.Pose::GetError(m_CsKFGT[iKFr], m_CsKFGT[m_Zp.m_iKFs[i]], i,
                            &xks[0][i], BA_ANGLE_EPSILON);
      }
    } else
#endif
    {
...
//#ifdef CFG_DEBUG
    ...
    for (int i = 0; i < 3; ++i) {
      if (xg[i].Valid()) {
        _e[i] = e;
        D.GetError(J, xg[i], xc1[i].m_ep, xc1[i].m_er, xm1[i].m_ev,
                   xm1[i].m_eba, xm1[i].m_ebw, xc2[i].m_ep, xc2[i].m_er,
...
//#if 0
  ...
  m_Zp.m_Acc[Nk][Nk].IncreaseDiagonal(
      UT::Inverse(BA_VARIANCE_PRIOR_POSITION_NEW,
                  BA_WEIGHT_PRIOR_CAMERA_INITIAL),
      UT::Inverse(BA_VARIANCE_PRIOR_ROTATION_NEW,
                  BA_WEIGHT_PRIOR_CAMERA_INITIAL));
  m_Zp.m_Amm.IncreaseDiagonal(
      UT::Inverse(BA_VARIANCE_PRIOR_VELOCITY_NEW,
                  BA_WEIGHT_PRIOR_CAMERA_INITIAL),
...
//#ifdef CFG_DEBUG
  ...
  for (int _ic = 1; _ic < icMax; ++_ic) {
    ...
    for (int iI = 0; iI < NI; ++iI) {
      ...
      for (int i = i1; i < i2; ++i) {
        const int iz2 = LF1.m_Zm.m_izms[i].m_iz2, ix = LF2.m_zs[iz2].m_ix,
                  iST = KF.m_ix2ST[ix];
...
//#if 1
  ...
  for (int iZ = 0; iZ < NZ; ++iZ) {
    ...
    for (int ix = 0; ix < Nx; ++ix) {
      ...
      KF.m_STs.insert(KF.m_STs.end(), m_STsTmp.begin() + iST1,
                      m_STsTmp.begin() + iST2);
      KF.m_usST.insert(KF.m_usST.end(), m_usSTTmp.begin() + iST1,
  ...
  if (nLFs2 > 1) {
    ...
    if (nLFs2 > 2) {
      ...
      IMU::PreIntegrate(_LF.m_us, m_LFs[__iLF].m_T.m_t, _t, m_CsLF[__iLF], &_D,
...
//#ifdef CFG_DEBUG
      ...
      UT::DebugStart();
#endif
      IMU::PreIntegrate(LF.m_us, _t, LF.m_T.m_t, _C, &m_DsLFGT[iLF], &m_work,
                        true, &_LF.m_us.Back(), NULL, BA_ANGLE_EPSILON);
//#ifdef CFG_DEBUG
#if 0
      UT::DebugStop();
      UT::PrintSeparator();
      ...
      if (nLFs2 > 2) {
        ...
        IMU::PreIntegrate(_LF.m_us, m_LFs[__iLF].m_T.m_t, _t, m_CsLFGT[__iLF],
                          &_D, &m_work, true, &_D.m_u1, &LF.m_us.Front(),
                          BA_ANGLE_EPSILON);
#ifdef LBA_DEBUG_GROUND_TRUTH_MEASUREMENT
...
#endif
  if (LF.m_T.m_iFrm == 0) {
    const LA::Vector3f s2r = LA::Vector3f::Get(
        BA_VARIANCE_FIX_ORIGIN_ROTATION_X, BA_VARIANCE_FIX_ORIGIN_ROTATION_Y,
...
#ifdef CFG_HANDLE_SCALE_JUMP
    m_dsKF[_iKF] = AverageDepths(m_ds.data() + m_iKF2d[_iKF],
                                 m_iKF2d[_iKF + 1] - m_iKF2d[_iKF]);
#endif
...
//#ifdef CFG_DEBUG
  ...
  m_GM->LBA_PushKeyFrame(GlobalMap::Camera(IKF.m_C.m_T, KF.m_T.m_iFrm,
...
#ifdef CFG_CHECK_REPROJECTION
  m_esKF.resize(nKFs2);
  ComputeErrorFeature(&KF, m_CsKF[nKFs1], m_CsKF, m_ds, &m_esKF[nKFs1].first,
                      nKFs1);
#endif
#ifdef LBA_DEBUG_VIEW
  if (g_viewer) {
  ...
  if (m_Zp.Pose::Invalid()) {
    ...
    if (m_LFs[iLF].m_T.m_iFrm == m_KFs[iKFr].m_T.m_iFrm) {
      ...
      m_Zp.Initialize(BA_WEIGHT_PRIOR_CAMERA_INITIAL, iKFr, m_CsKF[iKFr],
                      ...
                      &m_CsLF[iLF].m_T, BA_VARIANCE_PRIOR_POSITION_NEW,
                      BA_VARIANCE_PRIOR_ROTATION_NEW);
#ifdef LBA_DEBUG_GROUND_TRUTH_MEASUREMENT
      if (m_CsGT) {
        m_Zp.Pose::DebugSetMeasurement(m_CsKFGT[iKFr], false,
...
//#ifdef CFG_DEBUG
  ...
  for (int i1 = 0, i2 = 0, iKF = 0; i1 < N; i1 = i2) {
    iKF = static_cast<int>(
              std::upper_bound(m_iKF2d.begin() + iKF, m_iKF2d.end(), ids[i1]) -
              m_iKF2d.begin()) -
          1;
    ...
    for (int i = i1; i < i2; ++i) {
      ...
      if (m_uds[id] & LBA_FLAG_TRACK_UPDATE_INFORMATION_ZERO) {
        m_uds[id] =
            LBA_FLAG_TRACK_INVALID | LBA_FLAG_TRACK_UPDATE_INFORMATION_ZERO;
      } else {
...
void LocalBundleAdjustor::UpdateCameras(const std::vector<ubyte> &ucs,
  ...
  for (int ic = 0; ic < nLFs; ++ic) {
    ...
    if (/*_iKFNearest == -1 || */ !(ucs[_iKFNearest] &
                                    GM_FLAG_FRAME_UPDATE_CAMERA)) {
      continue;
    }
...
void LocalBundleAdjustor::MarkFeatureMeasurements(const LocalFrame &LF,
                                                  const int iKF,
                                                  std::vector<int> &ix2z) {
...
#ifdef CFG_DEBUG
void LocalBundleAdjustor::DebugSetFeatureMeasurements(
