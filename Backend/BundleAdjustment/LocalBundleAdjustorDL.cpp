    ...
    for (int iZ = 0; iZ < NZ; ++iZ) {
      ...
      *Tr = C / m_CsKF[iKF];
      ...
      for (int iz = Z.m_iz1; iz < Z.m_iz2; ++iz) {
        ...
        const int ix = z.m_ix;
        const float *xd =
            xds && (uds[ix] & LBA_FLAG_TRACK_UPDATE_DEPTH) ? &xds[ix] : NULL;
        if (!xc && !xd) {
          continue;
        }
        FTR::GetReduction(LF.m_Lzs[iz], Tr, KF.m_xs[ix], ds[ix], z, xc, xd, Ra,
                          Rp);
        dFa = Ra.m_dF + dFa;
        dFp = Rp.m_dF + dFp;
//#ifdef CFG_DEBUG
    ...
    for (int iZ = 0; iZ < NZ; ++iZ) {
      ...
      *Tr = C / m_CsKF[_iKF];
      ...
      for (int iz = Z.m_iz1; iz < Z.m_iz2; ++iz) {
        ...
        FTR::GetReduction(KF.m_Azs[iz], Tr, _KF.m_xs[ix], ds[ix], z, xds[ix],
                          Ra, Rp);
//#ifdef CFG_DEBUG
#if 0
        if (iKF == 43) {
...
void LocalBundleAdjustor::ComputeReductionPriorCameraMotion() {
  ...
  m_ZpLF.GetReduction(BA_WEIGHT_PRIOR_CAMERA_MOTION, m_ApLF, m_CsLF[iLF], _xr,
                      _xv, _xba, _xbw, &Ra, &Rp);
  m_dFa = Ra.m_dF + m_dFa;
  m_dFp = Rp.m_dF + m_dFp;
}
...
#endif
    m_DsLF[iLF2].GetReduction(BA_WEIGHT_IMU, m_AdsLF[iLF2], m_CsLF[iLF1],
                              m_CsLF[iLF2], m_K.m_pu, _xp[r1], _xr[r1], _xv[r1],
...
void LocalBundleAdjustor::ComputeReductionFixPositionZ() {
  ...
  for (int ic = 0; ic < Nc; ++ic) {
    ...
    z.GetReduction(A, m_CsLFBkp[iLF].m_p.z(), m_CsLF[iLF].m_p.z(), xcs[ic].v2(),
                   Ra, Rp);
    m_dFa = Ra.m_dF + m_dFa;
    m_dFp = Rp.m_dF + m_dFp;
  }
...
//#ifdef CFG_DEBUG
      ...
      d.u() += xds[ix];
      d.s2() =
          DEPTH_VARIANCE_EPSILON + KF.m_Mxs[ix].m_mdd.m_a * BA_WEIGHT_FEATURE;
      // if (!d.Valid()) {
      //  d = dsBkp[ix];
      //  continue;
      //}
  ...
  if (m_verbose >= 3) {
    ...
    UT::Print("         + %d / %d = %.2f%%\n", SNm, Nc,
              UT::Percentage(SNm, Nc));
    UT::Print("  Depth = %3d / %5d = %.2f%% (%d / %d = %.2f%%)\n", SNx, Nd,
              UT::Percentage(SNx, Nd), SNX, nKFs, UT::Percentage(SNX, nKFs));
...
bool LocalBundleAdjustor::UpdateStatesDecide() {
  ...
  for (int ic1 = 0, ic2 = 1; ic2 < nLFs; ic1 = ic2++) {
    ...
    IMU::PreIntegrate(m_LFs[iLF2].m_us, m_LFs[iLF1].m_T.m_t,
                      m_LFs[iLF2].m_T.m_t, m_CsLF[iLF1], &D, &m_work, true,
...
void LocalBundleAdjustor::ConvertCameraUpdates(const float *xcs,
...
void LocalBundleAdjustor::ConvertDepthUpdates(const float *xs,
...
float LocalBundleAdjustor::AverageDepths(const Depth::InverseGaussian *ds,
                                         const int N) {
