...
class AlignedMatrix2x4f {
 public:
  ...
  inline bool AssertEqual(const AlignedMatrix2x4f& M, const int verbose = 1,
    ...
    if (UT::VectorAssertEqual(&m00(), &M.m00(), 8, verbose, str, epsAbs,
                              epsRel)) {
      return true;
    } else if (verbose) {
      UT::PrintSeparator();
...
class EigenMatrix2x4f : public Eigen::Matrix<float, 2, 4> {
 public:
  ...
  inline bool AssertEqual(const LA::AlignedMatrix2x4f& M, const int verbose = 1,
                          const std::string str = "", const float epsAbs = 0.0f,
                          const float epsRel = 0.0f) const {
  ...
  inline bool AssertEqual(const EigenMatrix2x4f& e_M, const int verbose = 1,
                          const std::string str = "", const float epsAbs = 0.0f,
                          const float epsRel = 0.0f) const {
    return AssertEqual(e_M.GetAlignedMatrix2x4f(), verbose, str, epsAbs,
                       epsRel);
  }
};
#endif
