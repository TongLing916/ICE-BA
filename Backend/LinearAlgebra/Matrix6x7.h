...
class AlignedMatrix6x7f {
 public:
  ...
  static inline void AddABToSymmetric(const AlignedMatrix3x3f& A0,
    ...
    AB.m_04_05_06_x() +=
        B.m_04_05_06_x() * t1 + B.m_14_15_16_x() * t2 + B.m_24_25_26_x() * t3;
    t1 = A0.m10();
    t2 = A0.m11();
    t3 = A0.m12();
  ...
  static inline void ATBToSymmetric(const AlignedMatrix3x6f& A,
    ...
    ATB.m_00_01_02_03() = B.m_00_01_02_03() * a0 + B.m_10_11_12_13() * a1 +
                          B.m_20_21_22_23() * a2;
    ATB.m_04_05_06_x() =
        B.m_04_05_06_x() * a0 + B.m_14_15_16_x() * a1 + B.m_24_25_26_x() * a2;
  ...
  inline bool AssertEqual(const AlignedMatrix6x7f& M, const int verbose = 1,
    ...
    if (UT::VectorAssertEqual(&m00(), &M.m00(), 7, verbose, str, epsAbs,
                              epsRel) &&
        UT::VectorAssertEqual(&m10(), &M.m10(), 7, verbose, str, epsAbs,
                              epsRel) &&
        UT::VectorAssertEqual(&m20(), &M.m20(), 7, verbose, str, epsAbs,
                              epsRel) &&
        UT::VectorAssertEqual(&m30(), &M.m30(), 7, verbose, str, epsAbs,
                              epsRel) &&
        UT::VectorAssertEqual(&m40(), &M.m40(), 7, verbose, str, epsAbs,
                              epsRel) &&
        UT::VectorAssertEqual(&m50(), &M.m50(), 7, verbose, str, epsAbs,
                              epsRel)) {
      return true;
    } else if (verbose) {
      UT::PrintSeparator();
...
class SymmetricMatrix6x7 {
 public:
  ...
  inline void operator-(const SymmetricMatrix6x7<TYPE>& B) const {
    ...
    for (int i = 0; i < 27; ++i) AmB.m_data[i] = m_data[i] - B.m_data[i];
    return AmB;
  }
  ...
  inline bool AssertEqual(const SymmetricMatrix6x7<TYPE>& A,
    ...
    if (UT::VectorAssertEqual<TYPE>(m_data, A.m_data, 27, verbose, str, epsAbs,
                                    epsRel)) {
      return true;
    } else if (verbose) {
      UT::PrintSeparator();
...
class EigenMatrix6x7f : public Eigen::Matrix<float, 6, 7> {
 public:
  ...
  inline bool AssertEqual(const LA::AlignedMatrix6x7f& M, const int verbose = 1,
                          const std::string str = "", const float epsAbs = 0.0f,
                          const float epsRel = 0.0f) const {
  ...
  inline bool AssertEqual(const EigenMatrix6x7f& e_M, const int verbose = 1,
                          const std::string str = "", const float epsAbs = 0.0f,
                          const float epsRel = 0.0f) const {
    return AssertEqual(e_M.GetAlignedMatrix6x7f(), verbose, str, epsAbs,
                       epsRel);
  }
};
#endif
