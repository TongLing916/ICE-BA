...
class AlignedMatrix4x4f {
 public:
  ...
  inline bool AssertEqual(const AlignedMatrix4x4f& M, const int verbose = 1,
    ...
    if (UT::VectorAssertEqual(&m00(), &M.m00(), 16, verbose, str, epsAbs,
                              epsRel)) {
      return true;
    } else if (verbose) {
      UT::PrintSeparator();
...
class SymmetricMatrix4x4 {
 public:
  ...
  static inline void AAT(const AlignedMatrix4x4f& A,
                         SymmetricMatrix4x4<TYPE>& AAT);
...
template <>
inline void SymmetricMatrix4x4f::Get(AlignedMatrix4x4f& M) const {
...
class EigenMatrix4x4f : public Eigen::Matrix4f {
 public:
  ...
  inline bool AssertEqual(const LA::AlignedMatrix4x4f& M, const int verbose = 1,
                          const std::string str = "", const float epsAbs = 0.0f,
                          const float epsRel = 0.0f) const {
  ...
  inline bool AssertEqual(const EigenMatrix4x4f& e_M, const int verbose = 1,
                          const std::string str = "", const float epsAbs = 0.0f,
                          const float epsRel = 0.0f) const {
    return AssertEqual(e_M.GetAlignedMatrix4x4f(), verbose, str, epsAbs,
                       epsRel);
  }
};
#endif
