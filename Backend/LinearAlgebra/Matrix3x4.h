...
class AlignedMatrix3x4f {
 public:
  ...
  inline bool AssertEqual(const AlignedMatrix3x4f& M, const int verbose = 1,
    ...
    if (UT::VectorAssertEqual(&m00(), &M.m00(), 12, verbose, str, epsAbs,
                              epsRel)) {
      return true;
    } else if (verbose) {
      UT::PrintSeparator();
  ...
  // TODO(haomin) : add comments to this function. This function doesn't have a
  // unit test yet.
...
class EigenMatrix3x4f : public Eigen::Matrix<float, 3, 4> {
 public:
  ...
  inline bool AssertEqual(const LA::AlignedMatrix3x4f& M, const int verbose = 1,
                          const std::string str = "", const float epsAbs = 0.0f,
                          const float epsRel = 0.0f) const {
  ...
  inline bool AssertEqual(const EigenMatrix3x4f& e_M, const int verbose = 1,
                          const std::string str = "", const float epsAbs = 0.0f,
                          const float epsRel = 0.0f) const {
    return AssertEqual(e_M.GetAlignedMatrix3x4f(), verbose, str, epsAbs,
                       epsRel);
  }
};
#endif
