...
class Vector2 {
 public:
  ...
  inline bool AssertEqual(const Vector2<TYPE> &v, const int verbose = 1,
    ...
    if (UT::VectorAssertEqual<TYPE>(&v0(), &v.v0(), 2, verbose, str, epsAbs,
                                    epsRel)) {
      return true;
    } else if (verbose) {
      UT::PrintSeparator();
  ...
  static inline void apb(const Vector2<TYPE> &a, const Vector2<TYPE> &b,
                         Vector2<TYPE> &apb) {
    apb.v0() = a.v0() + b.v0();
    apb.v1() = a.v1() + b.v1();
  }
  static inline void amb(const Vector2<TYPE> &a, const Vector2<TYPE> &b,
                         Vector2<TYPE> &amb) {
    amb.v0() = a.v0() - b.v0();
    amb.v1() = a.v1() - b.v1();
  }
...
template <>
inline void Vector2f::Set(const float *v) {
  memcpy(this, v, sizeof(Vector2f));
}
template <>
inline void Vector2d::Set(const double *v) {
  memcpy(this, v, sizeof(Vector2d));
}
template <>
inline void Vector2f::Set(const double *v) {
  v0() = static_cast<float>(v[0]);
  v1() = static_cast<float>(v[1]);
}
template <>
inline void Vector2d::Set(const float *v) {
  v0() = static_cast<double>(v[0]);
  v1() = static_cast<double>(v[1]);
}
template <>
inline void Vector2f::Get(float *v) const {
  memcpy(v, this, sizeof(Vector2f));
}
template <>
inline void Vector2d::Get(double *v) const {
  memcpy(v, this, sizeof(Vector2d));
}
template <>
inline void Vector2f::Get(double *v) const {
  v[0] = static_cast<double>(v0());
  v[1] = static_cast<double>(v1());
}
template <>
inline void Vector2d::Get(float *v) const {
