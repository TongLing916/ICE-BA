...
class AlignedMatrix2x2f {
 public:
  ...
  inline bool AssertEqual(const AlignedMatrix2x2f &M, const int verbose = 1,
    ...
    if (UT::VectorAssertEqual(&m00(), &M.m00(), 4, verbose, str, epsAbs,
                              epsRel)) {
      return true;
    } else if (verbose) {
      UT::PrintSeparator();
  ...
  static inline void Ab(const AlignedMatrix2x2f &A, const Vector2f &b,
                        Vector2f &Ab) {
    const xp128f t =
  ...
  static inline void AddAbTo(const AlignedMatrix2x2f &A, const float *b,
                             Vector2f &Ab) {
  ...
  static inline void ABT(const AlignedMatrix2x2f &A, const AlignedMatrix2x2f &B,
...
class SymmetricMatrix2x2 {
 public:
  ...
  inline bool GetInverseLDL(SymmetricMatrix2x2<TYPE> &MI,
                            const TYPE *eps) const {
    TYPE M[2][2];
    Get(M);
    TYPE *_M[2] = {M[0], M[1]};
  ...
  static inline SymmetricMatrix2x2<TYPE> GetRandom(const TYPE mMin,
                                                   const TYPE mMax) {
    SymmetricMatrix2x2<TYPE> v;
    v.Random(mMin, mMax);
    return v;
  ...
  static inline void AB(const AlignedMatrix2x2f &A, SymmetricMatrix2x2<TYPE> &B,
                        AlignedMatrix2x2f &AB) {
    AB.m00() = A.m00() * B.m00() + A.m01() * B.m10();
    AB.m01() = A.m00() * B.m01() + A.m01() * B.m11();
    AB.m10() = A.m10() * B.m00() + A.m11() * B.m10();
    AB.m11() = A.m10() * B.m01() + A.m11() * B.m11();
  }
  static inline void AAT(const AlignedMatrix2x2f &A,
                         SymmetricMatrix2x2<TYPE> &AAT) {
...
class EigenMatrix2x2f : public Eigen::Matrix2f {
 public:
  ...
  inline bool AssertEqual(const LA::AlignedMatrix2x2f &M, const int verbose = 1,
                          const std::string str = "", const float epsAbs = 0.0f,
                          const float epsRel = 0.0f) const {
  ...
  inline bool AssertEqual(const EigenMatrix2x2f &e_M, const int verbose = 1,
                          const std::string str = "", const float epsAbs = 0.0f,
                          const float epsRel = 0.0f) const {
    return AssertEqual(e_M.GetAlignedMatrix2x2f(), verbose, str, epsAbs,
                       epsRel);
  }
  inline bool AssertZero(const int verbose = 1, const std::string str = "",
                         const float epsAbs = 0.0f,
