...
class AlignedVector3f {
 public:
  ...
  inline void Set(const double *v) {
    v012r().vset_all_lane(static_cast<float>(v[0]), static_cast<float>(v[1]),
  ...
  inline float SquaredLength() const { return (v012r() * v012r()).vsum_012(); }
  ...
  inline void Print(const std::string str, const bool e, const bool l,
                    const bool n) const {
  ...
  inline bool AssertEqual(const AlignedVector3f &v, const int verbose = 1,
    ...
    if (UT::VectorAssertEqual(&v0(), &v.v0(), 3, verbose, str, epsAbs,
                              epsRel)) {
      return true;
    } else if (verbose) {
      UT::PrintSeparator();
...
class Vector3 {
 public:
  ...
  inline bool AssertEqual(const Vector3<TYPE> &v, const int verbose = 1,
    ...
    if (UT::VectorAssertEqual<TYPE>(&v0(), &v.v0(), 3, verbose, str, epsAbs,
                                    epsRel)) {
      return true;
    } else if (verbose) {
      UT::PrintSeparator();
...
template <>
inline void Vector3f::Set(const float *v) {
  memcpy(this, v, sizeof(Vector3f));
}
template <>
inline void Vector3d::Set(const double *v) {
  memcpy(this, v, sizeof(Vector3d));
}
template <>
inline void Vector3f::Set(const double *v) {
