...
class AlignedVector6f {
 public:
  ...
  inline bool AssertEqual(const LA::AlignedVector6f &v, const int verbose = 1,
    ...
    if (UT::VectorAssertEqual(&v0(), &v.v0(), 6, verbose, str, epsAbs,
                              epsRel)) {
      return true;
    } else if (verbose) {
      UT::PrintSeparator();
 ...
 public:
  ...
  static inline void AddsaTo(const float s, const AlignedVector6f &a,
                             AlignedVector6f &sa) {
    sa.v0123() += a.v0123() * s;
    sa.v4() += a.v4() * s;
    sa.v5() += a.v5() * s;
...
class Vector6 {
 public:
  ...
  inline bool AssertEqual(const Vector6<TYPE> &v, const int verbose = 1,
                          const std::string str = "", const float epsAbs = 0.0f,
                          const float epsRel = 0.0f) const {
    if (UT::VectorAssertEqual<TYPE>(&v0(), &v.v0(), 6, verbose, str, epsAbs,
                                    epsRel)) {
      return true;
    } else if (verbose) {
      UT::PrintSeparator();
...
template <>
inline void Vector6f::Set(const float *v) {
  memcpy(this, v, sizeof(Vector6f));
}
template <>
inline void Vector6f::Set012(const float *v) {
  memcpy(&v0(), v, 12);
}
template <>
inline void Vector6f::Set345(const float *v) {
  memcpy(&v3(), v, 12);
}
// TODO (yanghongtian) : check this operation.
template <>
inline void Vector6f::Get012(float *v) const {
  memcpy(v, &v0(), 12);
}
template <>
inline void Vector6f::Get345(float *v) const {
  memcpy(v, &v3(), 12);
}

template <>
inline void Vector6d::Set(const float *v) {
  for (int i = 0; i < 6; ++i) {
    m_data[i] = static_cast<double>(v[i]);
  }
}
template <>
inline void Vector6d::Set(const double *v) {
  memcpy(this, v, sizeof(Vector6d));
}
template <>
inline void Vector6d::Get012(float *v) const {
  v[0] = static_cast<float>(v0());
  v[1] = static_cast<float>(v1());
  v[2] = static_cast<float>(v2());
}
template <>
inline void Vector6d::Get345(float *v) const {
...
class EigenVector6f : public Eigen::Matrix<float, 6, 1> {
 public:
  ...
  inline EigenVector6f(const LA::AlignedVector3f &v0,
