...
class Measurement {
 ...
 public:
  ...
  inline void Set(const int ix, const Point2D &z,
...
class ES {
 public:
  ...
  inline void Accumulate(const Intrinsic &K, const LA::Vector2f &ex,
                         const float F, const ESIndex idx,
                         const bool r = true) {
    m_ESx.Accumulate(ESError(K, ex), F, idx, r);
  }
  inline void Accumulate(const float ed, const float F, const ESIndex idx,
                         const bool r = true) {
...
class DDC {
 public:
  ...
  inline bool AssertEqual(const DDC &a, const int verbose = 1,
                          const std::string str = "", const float epsAbs = 0.0f,
                          const float epsRel = 0.0f) const {
    return m_adc.AssertEqual(a.m_adc, verbose, str + ".m_adc", epsAbs,
                             epsRel) &&
  ...
  static inline void aTb(const float *a, const DDC &b,
                         Camera::Factor::Unitary::CC &aTb) {
...
class A1 {
 public:
  ...
  inline bool AssertEqual(const A1 &A, const int verbose = 1,
                          const std::string str = "", const float epsAbs = 0.0f,
                          const float epsRel = 0.0f) const {
    return m_adcz.AssertEqual(A.m_adcz, verbose, str + ".m_adcz", epsAbs,
                              epsRel);
  }
...
inline void DebugSetMeasurement(const Rigid3D *T12, const Source &x1,
                                const Depth::InverseGaussian &d1,
                                Measurement &z2) {
#ifdef CFG_STEREO
  if (z2.m_z.Valid()) {
...
inline void GetError(const Rigid3D &T12, const Source &x1,
                     const Depth::InverseGaussian &d1, const Point2D &z2,
                     LA::Vector2f &e2) {
#ifdef CFG_DEBUG
  UT_ASSERT(z2.Valid());
#endif
...
inline void GetError(const ErrorJacobian::D &Je, const float xd,
                     LA::Vector2f &e) {
  e = Je.m_e;
  e += Je.m_Jd * xd;
}
...
inline void GetErrorJacobian(const Rigid3D &T12, const Source &x1,
                             const Depth::InverseGaussian &d1,
                             const Rigid3D &T2, const Point2D &z2,
                             ErrorJacobian::D &Je2
#ifdef CFG_STEREO
                             ,
...
inline void GetFactor(const float w, const Rigid3D *T12, const Source &x1,
                      const Depth::InverseGaussian &d1, const Rigid3D &T2,
                      const Measurement &z2, Factor::Depth *A,
                      Factor::Depth::U *U,
#ifdef CFG_STEREO
                      const Point3D &br,
#endif
                      const float r2Max = FLT_MAX) {
  GetFactor<ME_FUNCTION, Factor::Depth, Factor::Depth::U>(w, T12, x1, d1, T2,
                                                          z2, A, U,
#ifdef CFG_STEREO
                                                          br,
#endif
...
inline void GetFactor(const float w, const Rigid3D *T12, const Source &x1,
  ...
  GetFactor<ME_FUNCTION, Factor::FixSource::L, Factor::FixSource::U>(
      w, T12, x1, d1, T2, z2, L, U,
#ifdef CFG_STEREO
      br,
#endif
      r2Max);
  U->m_A.Get(A2->m_add.m_a, A1->m_adcz, A2->m_add.m_b, A2->m_Aczz.m_A,
             A2->m_Aczz.m_b);
}
template <int ME_FUNCTION>
inline void GetFactor(const float w, const Rigid3D *T12, const Source &x1,
                      const Depth::InverseGaussian &d1, const Rigid3D &T2,
                      const Measurement &z2, Factor::Full::L *L,
                      Factor::Full::A1 *A1, Factor::Full::A2 *A2,
                      Factor::Full::U *U,
#ifdef CFG_STEREO
                      const Point3D &br,
#endif
                      const float r2Max = FLT_MAX) {
  GetFactor<ME_FUNCTION, Factor::Full::L, Factor::Full::U>(w, T12, x1, d1, T2,
                                                           z2, L, U,
#ifdef CFG_STEREO
                                                           br,
#endif
                                                           r2Max);
  U->m_A.Get(A2->m_adx.m_add.m_a, A2->m_adx.m_adc, A1->m_adcz,
             A2->m_adx.m_add.m_b, A2->m_Acxx.m_A, A2->m_Acxz, A2->m_Acxx.m_b,
             A2->m_Aczz.m_A, A2->m_Aczz.m_b);
}

template <class LINEARIZATION>
inline float GetCost(const LINEARIZATION &L, const Measurement &z,
                     const Error &e) {
#ifdef CFG_STEREO
  float F = 0.0f;
  if (z.m_z.Valid()) {
...
inline float GetCost(const Factor::Depth &A, const Measurement &z,
...
inline void GetReduction(const Factor::Depth &A, const Rigid3D *T12,
                         ...
                         const Measurement &z2, const float xd, Reduction &Ra,
                         Reduction &Rp) {
...
inline void GetReduction(const Factor::FixSource::L &L, const Rigid3D *T12,
                         const Source &x1, const Depth::InverseGaussian &d1,
                         const Measurement &z2, const LA::ProductVector6f *xcz,
...
inline void GetReduction(const Factor::Full::L &L, const Rigid3D *T12,
                         const Source &x1, const Depth::InverseGaussian &d1,
                         const Measurement &z2, const LA::ProductVector6f *xcx,
                         const LA::ProductVector6f *xcz, const float *xd,
...
#ifdef CFG_STEREO
#ifdef CFG_DEBUG
inline void DebugSetMeasurement(const Point3D &br,
                                const Depth::InverseGaussian &d, Source &x) {
...
inline float GetCost(const Factor::Stereo &A, const Source &x,
                     const LA::Vector2f &e) {
  return A.m_w * LA::SymmetricMatrix2x2f::MahalanobisDistance(x.m_Wr, e);
}
inline float GetCost(const Factor::Stereo &A, const Source &x, const float xd,
                     LA::Vector2f &e) {
  GetError(A, xd, e);
...
class EigenErrorJacobian {
 ...
 public:
  ...
  inline void AssertEqual(const ErrorJacobian::DCXZ &Je, const int verbose = 1,
                          const std::string str = "", const float epsAbs = 0.0f,
                          const float epsRel = 0.0f) const {
    m_Jd.AssertEqual(Je.m_Jd, verbose, str + ".m_Jd", epsAbs, epsRel);
...
class EigenFactor {
  ...
  class DDC {
   public:
    ...
    inline bool AssertEqual(const Factor::DDC &a, const int verbose = 1,
                            ...
                            const float epsAbs = 0.0f,
                            const float epsRel = 0.0f) const {
      return m_add.AssertEqual(a.m_add, verbose, str + ".m_add", epsAbs,
                               epsRel) &&
             Factor::DC::Get(m_adc.GetVector6f())
                 .AssertEqual(a.m_adc, verbose, str + ".m_adc", epsAbs, epsRel);
    }
  ...
  class Stereo {
   public:
    ...
    inline void AssertEqual(const Factor::Stereo &A, const int verbose = 1,
      ...
      UT::AssertEqual(m_add, A.m_add.m_a, verbose, str + ".m_add", epsAbs,
                      epsRel);
      UT::AssertEqual(m_bd, A.m_add.m_b, verbose, str + ".m_bd", epsAbs,
                      epsRel);
    }
 ...
 public:
  inline EigenFactor() {}
  inline EigenFactor(const float F,
                     const Eigen::Matrix<float, 13, 14, Eigen::RowMajor> &A) {
    m_F = F;
    m_add = A(0, 0);
  ...
  inline void AssertEqual(const Factor::Full::L &L, const Factor::Full::A1 &A1,
                          const Factor::Full::A2 &A2, const int verbose = 1,
                          const std::string str = "", const float epsAbs = 0.0f,
                          const float epsRel = 0.0f) const {
    UT::AssertEqual(m_F, L.m_F, verbose, str + ".m_F");
    Factor::DD::Get(m_add, m_bd)
        .AssertEqual(A2.m_adx.m_add, verbose, str + ".m_add", epsAbs, epsRel);
    Factor::DC::Get(m_adcx.GetVector6f())
        .AssertEqual(A2.m_adx.m_adc, verbose, str + ".m_adcx", epsAbs, epsRel);
    Factor::DC::Get(m_adcz.GetVector6f())
        .AssertEqual(A1.m_adcz, verbose, str + ".m_adcz", epsAbs, epsRel);
...
EigenErrorJacobian EigenGetErrorJacobian(const Rigid3D &C1, const Source &x1,
                                         const Depth::InverseGaussian &d1,
                                         const Rigid3D &C2, const Point2D &z2,
                                         const bool cx, const bool cz
#ifdef CFG_STEREO
                                         ,
                                         const Point3D *br = NULL
#endif
...
inline EigenFactor EigenGetFactor(const float w, const Rigid3D &C1,
                                  const Source &x1,
                                  const Depth::InverseGaussian &d1,
                                  const Rigid3D &C2, const Measurement &z2,
                                  const bool cx, const bool cz
#ifdef CFG_STEREO
                                  ,
                                  const Point3D &br
