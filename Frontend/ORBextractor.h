...
class ORBextractor {
 public:
  ...
  static void HarrisResponses(const cv::Mat& img, int blockSize, float harris_k,
                              vector<KeyPoint>* pts);
  ...
  static inline void accumulate_matrix_coeff(const uint8x16_t* top_row,
                                             ...
                                             const int block_size, int32x4_t* a,
                                             int32x4_t* b, int32x4_t* c) {
    union {
      int16x8_t ix;
      int16_t ixc[8];
      ...
      *row_bot = vld1q_u8(ptr0 - 1);         // middle row
      ...
      for (int k = 0; k < blockSize; k++, ptr0 += step) {
        ...
        *row_bot = vld1q_u8(ptr0 + step - 1);  // bottom row
        accumulate_matrix_coeff(row_top, row_mid, row_bot, blockSize, &va, &vb,
                                &vc);
        if (blockSize == 9) {
          ...
          int ix = (rmid_ptr[10] - rmid_ptr[8]) * 2 +
                   (rtop_ptr[10] - rtop_ptr[8]) + (rbot_ptr[10] - rbot_ptr[8]);
          int iy = (rbot_ptr[9] - rtop_ptr[9]) * 2 +
                   (rbot_ptr[8] - rtop_ptr[8]) + (rbot_ptr[10] - rtop_ptr[10]);
          a += ix * ix;
          b += iy * iy;
          c += ix * iy;
        }
