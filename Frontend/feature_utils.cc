...
inline cv::Mat fast_mask_pyra_down(const cv::Mat& mask) {
  ...
  for (int y = 0; y < mask_small.rows; y++) {
    for (int x = 0; x < mask_small.cols; x++) {
      ...
      if (*(mask.data + shift0) == 0x00 || *(mask.data + shift1) == 0x00 ||
...
inline bool refine_kp_in_larger_img(
  ...
  for (int kp_small_idx = 0; kp_small_idx < kp_in_small_img.size();
    ...
    if (local_kps.size() > 0) {
      ORBextractor::HarrisResponses(img_in_smooth, harris_block_size, 0.04f,
                                    &local_kps);
      float score_total = 0;
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    for (int deg = 30; deg < 90; deg += 5) {
      ...
      if (dist_pt[0].x > 0 && dist_pt[0].y > 0 &&
          dist_pt[0].x < mask_size.width && dist_pt[0].y < mask_size.height) {
        viewable_degs[i] = deg;
      } else {
        break;
      }
...
bool detect_orb_features(
  ...
  // position at level 0.
  // For now, all the features are *refined* to pyramid0 as octave is 0 for all
  // detected features
  ...
  if (enforce_uniformity_radius > 5 &&
    ...
    // Basically, this weight_LUT can suppress at most a radius of 15 pixels.
    // In pracice,
    // it is possible to see features that are 2 to 4 pixels apart without being
    // suppressed.
    cv::Mat weight_LUT = cv::Mat::zeros(2 * 16 - 1, 2 * 16 - 1, CV_32F);
    for (int x = 0; x < 2 * 16 - 1; ++x) {
      for (int y = 0; y < 2 * 16 - 1; ++y) {
        weight_LUT.at<float>(y, x) = std::max(
            1 -
                static_cast<float>((15 - x) * (15 - x) + (15 - y) * (15 - y)) /
  ...
  if (VLOG_IS_ON(4)) {
    ...
    for (const auto& kp : kp_in_pyramids[det_pyra_level]) {
      ...
      VLOG(2) << "kp " << kp.pt << " at level " << det_pyra_level << " score "
              << kp.response;
    }
    cv::imwrite("/tmp/img_level_" +
                    boost::lexical_cast<std::string>(det_pyra_level) + ".png",
                small_debug);
  }
#endif
  // Refine the corner response.  Push keypoints from higher pyramids to pyramid
  // 0.
  ...
  for (int it_pyra = det_pyra_level; it_pyra > 0; it_pyra--) {
    refine_kp_in_larger_img(img_pyramids[it_pyra - 1], kp_in_pyramids[it_pyra],
    ...
    if (VLOG_IS_ON(4)) {
      ...
      cv::imwrite("/tmp/img_level_" +
                      boost::lexical_cast<std::string>(it_pyra - 1) + ".png",
                  small_debug);
    }
#endif
  }
  ...
  // descriptors.
  // The feat_half_size is large enough to satisfy the harris margin even after
  // moving up
  ...
  for (int i = 0, count = 0;
    ...
    // TODO(mingyu): use 0 as we have refined to pyra0 or use the real octave at
    // detection
    kp.octave = 0;
  ...
  for (int it_slave = 0; it_slave < pixels_slave.rows; ++it_slave) {
    ...
    if (x >= img_margin && y >= img_margin &&
      ...
      VLOG(2) << "it_slave " << it_slave << " slave_avg " << slave_avg
              << " slave x y [" << x << ", " << y << "]";
#endif
      // allow avg value diff 2x max pixel diff since the exposure of left and
      // right cam
      ...
      for (int v = -half_block_size_; v < half_block_size_; ++v) {
        for (int u = -half_block_size_; u < half_block_size_; ++u) {
          int slave_val = static_cast<int>(
              slave_image.data[(y + v * scale) * slave_col + x + u * scale]);
          int master_val = static_cast<int>(
              master_image.data[(master_y + v * scale) * master_col + master_x +
                                u * scale]);
...
bool SlaveImgFeatureDetector::detect_features_on_slave_img(
  ...
  for (size_t it_master = 0; it_master < master_pnts_undistorted.size();
    ...
    VLOG(2) << "before coarse det it_master " << it_master << " kp id "
            << master_kps[it_master].class_id << " master pixel [" << master_x
            << ", " << master_y << "]";
#endif
    if (!detect_features_on_slave_img_helper(
            master_image, slave_image, duo_calib_param,
            max_pixel_val_diff * 2,  // coarse
            master_x, master_y, s_R_m, s_t_m, pnt_master, coarse_search_range,
#ifdef DEBUG_DETECT_FEATURES_ON_SLAVE_IMG
            &slave_image_debug,
#endif
...
// 255);
...
#endif
      // if the sampled pos are dense, a couple of samples may get close to the
      // true position
...
#ifndef __FEATURE_UTILS_NO_DEBUG__
        VLOG(2)
            << "thresh test failed min_patch_diff2 / second_min_patch_diff2 "
            << min_patch_diff2 << " / " << second_min_patch_diff2 << " = "
    ...
    // ->
    const float Y = (max_search_dis - min_search_dis * search_range_fine.rows) /
    ...
    cv::putText(slave_image_debug, boost::lexical_cast<std::string>(
                ...
                cv::Point2i(best_slave_x, best_slave_y), cv::FONT_HERSHEY_PLAIN,
                1, 255, 1);
#endif
