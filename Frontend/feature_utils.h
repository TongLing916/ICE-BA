...
class FeatureTrackDetector {
 public:
  ...
  bool optical_flow_and_detect(
      ...
      std::vector<cv::KeyPoint>* key_pnts_ptr, cv::Mat* orb_feat_ptr = nullptr,
 ...
 public:
  ...
  enum { BUILD_TO_CURR = 0, BUILD_TO_PREV = 1 };

  void build_img_pyramids(const cv::Mat& img_in_smooth,
                          int build_type = BUILD_TO_CURR) {
    if (build_type == BUILD_TO_CURR) {
      build_pyramids(img_in_smooth, kMaxPyraLevelOF, &curr_img_pyramids_,
                     curr_pyramids_buffer_.get());
    } else {
      build_pyramids(img_in_smooth, kMaxPyraLevelOF, &prev_img_pyramids_,
...
class SlaveImgFeatureDetector {
 ...
 private:
  bool detect_features_on_slave_img_helper(
      const cv::Mat& master_image, const cv::Mat& slave_image,
      const DuoCalibParam& duo_calib_param, int max_pixel_val_diff,
      int master_x, int master_y, const cv::Mat_<float>& s_R_m,
      const cv::Mat_<float>& s_t_m, const cv::Mat_<float>& pnt_master,
...
bool align2D(const cv::Mat& cur_img, uint8_t* ref_patch_with_border,
             uint8_t* ref_patch, const int n_iter,
             Eigen::Vector2f* cur_px_estimate, bool no_simd = false);
...
bool warpAffine(const Eigen::Matrix2f& A_cur_ref, const cv::Mat& img_ref,
                const Eigen::Vector2f& px_ref, const int level_ref,
...
class DirectMatcher {
 public:
  ...
  struct Options {
    ...
    int max_iter;  //!< number of iterations for aligning the feature patches in
                   //!gauss newton
    float max_epi_length_optim;  //!< max length of epipolar line to skip
                                 //!epipolar search and directly go to img align
                                 //!NOLINT
  ...
  // [NOTE] This function assumes that px_cur is already set to an estimate that
  // is within
  ...
  bool findMatchDirect(const vio::cameras::CameraBase& cam_ref,
                       ...
                       const bool edgelet_feature, Eigen::Vector2f* px_cur);
...
bool detect_orb_features(
    const cv::Mat& img_in_raw, const cv::Mat_<uchar>& mask,
