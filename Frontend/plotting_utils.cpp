...
PoseDrawer3D::PoseDrawer3D(float viz_cam_height, const cv::Matx33f& cam_K)
    : img3d_{cv::viz::WImage3D(cv::Mat(1, 1, CV_8UC1),
                               cv::Size2f(0.16, 0.12))},  // placeholder
  ...
  cv::viz::WGrid grid(cv::Vec2i(20, 20), cv::Vec2i(1, 1),
                      cv::viz::Color::white());
...
void PoseDrawer3D::viz3d_once(const cv::Affine3f& W_T_D, const cv::Mat& img,
  ...
  const cv::Vec3f cam_xyz(W_T_D.translation()[0], W_T_D.translation()[1],
                          W_T_D.translation()[2]);
  ...
  if (viz3d_view_mode_ != CAM_FOLLOW) {
    ...
    if (!rig_xyz_mat.empty()) {
      ...
      cv::viz::WTrajectory traj(rig_xyz_affine3f, cv::viz::WTrajectory::PATH,
      ...
      for (int x = 0; x < depth_img.cols; ++x) {
        for (int y = 0; y < depth_img.rows; ++y) {
          ...
          if (z_val > far_cut) {
            ...
            color(y, x)[1] = static_cast<uint8_t>(255 * z_val / far_cut);
            color(y, x)[0] = 0;
          }
        }
      }
  ...
  if (viz3d_view_mode_ == TOP_VIEW_FOLLOW) {
    ...
    viz_window_ptr_->setViewerPose(cv::viz::makeCameraPose(
        cam_xyz * 0.5 + cv::Vec3f(0, 0, viz_cam_height_ + 0.5), cam_xyz * 0.5,
        cv::Vec3f(0, -1, 0)));
  } else if (viz3d_view_mode_ == CAM_FOLLOW) {
